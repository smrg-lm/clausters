...
- **The hit area is at least the drawn area.** Small targets (a breakpoint, a cord end, a handle) carry a slop role around the geometry; `interact.rs` and the layout must read the same rects, or clicking and drawing disagree in exactly the cases nobody tests.
