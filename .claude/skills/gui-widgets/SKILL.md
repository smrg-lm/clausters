...
- **Rulers** (`src/host/ruler.rs`, pure): an adaptive 1-2-5 time axis under the view (`ruler` prop: `"time"` default with `h:mm:ss.mmm` labels, `"samples"`, `"off"`); the spectrogram adds a Hz ruler at the left edge — decade ticks with 2×/5× subdivisions placed by inverting the shader's `bin_norm = f_lo^(1-d)` mapping (log) or linearly. Bitmap font + mesh lines.
