...
The crate-level model — metrics roles, the main-axis resolution order, the logical/physical seam — is in [[clausters-gui]]; it is staged on the L track, so check `clients/gui/PLAN.md` for which half has landed. What is per-element is deciding your kind's own size, and these rules settle it:
