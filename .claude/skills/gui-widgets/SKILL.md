...
The crate-level model — metrics roles, the main-axis resolution order, the logical/physical seam — is in [[clausters-gui]]; it is staged on the L track, so check `clients/gui/PLAN.md` for which half has landed. The **metrics table has landed** (`host/metrics.rs`, passed in beside the theme), so a size your widget needs is a role you read, never a literal you invent — and if no role means what you mean, the rule is the one below on shared vocabulary, not a fresh constant at the paint site. What is per-element is deciding your kind's own size, and these rules settle it:
