...
- **Native shell:** `transport` (UDP front), `client` (the UDP server leg), `shm` (read-only mmap of the server's `--shm` segment - control buses, `sample_clock()`, `sample_rate()`; validates `MAGIC`/`ABI_VERSION`), `mapfile`/`bulk` (mmap), `store`, `embed` (the `standalone` `EmbedServer`), `gui/` (winit + wgpu windowed front, multi-window by def id, ~30 fps animation for live views; split by role: `app` state + event loop, `windows` lifecycle, `serverleg` audio-server replies, `input` pointer gestures, `roll` piano-roll editing/MIDI).
