...
2. **El costo por muestra — medido: 0,010% de un bloque por lector.** Una
   ...
   **Las dos respuestas, en orden.** Primero, y gratis: **coser sólo lo que de
   ...
   Segundo, **hecho el 2026-09-11**: resolver el tramo una vez por corrida en
   vez de por muestra. `Buffer::run_at(frame)` contesta la corrida contigua en
   la que cae un frame —un tramo de un cosido, el largo entero de un buffer
   plano— y el `Reader` de `dsp::buf` la sostiene a lo largo del bloque,
   preguntando de nuevo sólo cuando un frame se le va. La búsqueda que saca eran
   **dos** preguntas y la corrida contesta las dos de una: qué tramo tiene el
   frame, y dónde guarda sus muestras la fuente de ese tramo (`PartRun`) — así
   que leer adentro de una corrida es la lectura indexada que cuesta un buffer
   plano. El camino por muestra queda de respaldo para la costura, un salto, una
   velocidad modulada y una fase que va para atrás.

   **Lo que midió**: por lectura, sostener la corrida en vez de resolver por
   muestra es **-50%** sobre un cosido (de un tramo o de 256, lo mismo). En un
   motor con 128 lectores: plano **5,0% → 4,2%** de un bloque —todo lector de
   buffers del servidor salió ganando, porque la misma corrida hace indexada
   también la lectura de un buffer plano— y cosidos **7,5% → 5,3%**, o sea que
   el sobrecosto de un join pasó de **+50% a +29%**. Con los crossfades apagados
   es **+16%**, que dice qué es lo que queda: el fade es la diferencia que
   sobra, y un fade es audio que alguien pidió y no sobrecarga.
