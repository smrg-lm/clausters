...
It also keeps a **shared transport** — a beat grid for phase-aligning several clients on that sample clock. `/transport` with no args **queries** it (replies `/transport.reply <int64 originSample> <double tempo> <int32 defined>`, `defined` 0 until one is set); `/transport <int64 originSample> <double tempo>` **sets** it (last writer wins, replies `/done`). Beat `b` is sample `originSample + b·rate/tempo`. It is an in-memory registry the server stores and serves but **never schedules from** — clients read it to start their routines on the same beat. See [`sample-clock.md`](sample-clock.md).
