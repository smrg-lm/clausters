...
A client does not *have* to enumerate the catalog — naming a kind and letting the server validate remains the normal path — but it **can**, with [`/u_query`](#def-buffer-and-ugen-introspection): the same descriptors as a typed reply, so a palette or completion list derives from the server's own truth (and from the build it is actually talking to) instead of a copy of this table that would drift. Input names below are given in **wire order**, which is what `/u_query` reports.
...
| `Shaper` | bufnum, signal | waveshaper: maps `signal` (in ±1, clamped) through a transfer table in wavetable format (typically `/b_gen cheby`); the table's first point is `signal = −1`, its last `signal = +1` |
...
| `XFade2` | a, b, pan, level | equal-power crossfade: −1 all `a`, 1 all `b`; keeps *uncorrelated* material at one loudness and lifts correlated material 3 dB in the middle |
...
| `SendTrig` | trig, id, value | on each trigger of `trig`, sends `/tr nodeID id value` to `/notify` clients; output is silence — see the side-effect note below |
...
| `Poll` | trig, signal, trig_id | on each trigger of `trig`, posts `label: value` (the `signal` value) to the server console (a static `label` field) and, when `trig_id ≥ 0`, also sends `/tr nodeID trig_id value`; passes `signal` through as its output |
| `FFT` | source, active | opens a spectral chain: windows `source` and transforms it to a spectral frame once per hop (`active > 0` runs, `≤ 0` holds); static fields `fft_size` (default 1024), `hop` (fraction, default 0.5), `wintype` (default 0 = Hann); the window is also settable live via `/u_cmd` — see the FFT-chain note below |
...
| `Conv` | source, kernel | partitioned convolution against a **prepared** kernel buffer (`/b_gen prepare_partconv`); static fields `fft_size` (transform size; the partition — and intrinsic latency — is `fft_size/2`) and `partitions` (the longest kernel accepted, default 16); see the convolution note below |
...
/b_query     bufnum...                 →  /b_info  bufnum frames channels sampleRate ...  (frames -1: empty slot)
/b_query     (no argument)              →  /b_info  every allocated buffer, same shape
...
`/b_query`, `/b_get` and `/b_getn` are **synchronous** reads, answered from the network-side buffer mirror (state as of the last completed command). `/b_query` with **no argument** lists every allocated buffer instead of querying named ones — see [Def, buffer and UGen introspection](#def-buffer-and-ugen-introspection). `/b_get` reads single samples by flat (interleaved) index; `/b_getn` reads ranges, with `count` clamped to what the buffer holds from `start` — a request past the end returns only the available samples, and an unallocated buffer returns count 0. Sample indices are flat across channels (`frame * channels + channel`), so a stereo buffer reads as interleaved `L R L R ...`. Large buffers are read in client-chosen chunks sized to the client's transport: over TCP/WebSocket a chunk may be as large as the `/server_info` frame ceiling (megabytes per round-trip), over UDP each reply must fit one datagram.
...
- **`/g_queryTree <groupID> [detail]`** → `/g_queryTree.reply` — the whole subtree from `groupID` (use `0` for the root). Args: `detail`, the group and its child count, then depth-first per node: ID and child count (`-1` marks a synth), the def name for synths, and per level the same payload `/n_info` carries. `detail` is scsynth's `flag` widened into a level: **0** the shape alone, **1** the control count followed by (name|index, value) pairs (both scsynth-compatible), **2** also the maps and the inferred bus lists — which is what makes every entry a **full node record**, so a client reading a tree needs no follow-up `/n_query`. Values above 2 clamp to 2.
- **`/n_query <nodeID>...`** → one `/n_info` per node — the same record for one node, plus where it sits. Layout: `nodeID, parentID, prevID, nextID, isGroup`; then for a **group** `headID, tailID` (`-1` if empty); for a **synth** `defName`, control count + (name|index, value) pairs, map count + (controlIndex, bus, audio) triples (the `/n_map`/`/n_mapa` bindings), and the inferred `reads`/`writes` bus lists as two strings (`"0,16"`, or `"-"` when none). Siblings are `-1` when absent.
  - A node the server does **not** hold answers `nodeID, -1, -1, -1, -1` — **`isGroup` = -1 means the node is gone** — rather than `/fail`. Absence is a *state* of the resource, reported in the record like `/d_query`'s empty family and `/b_query`'s `frames = -1`, so one dead id never aborts a query about the others. Only a malformed request (a non-int id) is a `/fail`.
...
Where the queries above read what is *playing*, these three read what the server **holds** — the material a client can build with. All are retrieval only: they add no semantics and change nothing.
...
- **`/d_query [name...]`** → one `/d_info` per def, then `/done "/d_query"`. With no argument, every loaded def of every family. Layout: `name, family, numControls`, then per control `name, default, rate`.
  - `family` is `"synth"`, `"faust"` or `"graph"`; a def the server does not hold comes back with an **empty** family and no controls, rather than failing the batch (the `/b_query` convention for an unallocated slot).
  - `rate` is the control type the def declared — `kr`, `tr` or `ir`, the same set the [control-type table](#control-types) defines, reported under the same field name `/d_recv` accepts it under. Note this is a **different vocabulary** from the calculation rates `/u_query` reports below: a control is a value pushed in from outside, so `ar` is not among them (map an audio bus with `In` + `/n_mapa` instead) and neither is `dr` (a demand value is *pulled* by a driver — there is nowhere for a `/n_set` to land). `tr` in turn is a behavior rather than a rate, which is why the server's own type is named `ControlType`.
...
- **`/b_query`** with **no argument** → one `/b_info` listing every **allocated** buffer, four args each (`bufnum, frames, channels, sampleRate`) — the same shape the per-index form replies with, so one parser reads both. (With arguments it is the per-index query documented under [Buffers](#buffers-b_).) The per-index form answers **`frames = -1`** for a slot with nothing in it, the same "absent is a state" rule `/n_query` and `/d_query` follow; the listing form simply omits it.
- **`/u_query [kind...]`** → one `/u_info` per UGen, then `/done "/u_query"`. With no argument, the whole catalog. Layout: `name, arity, defaultRate, rates, exec, bus, needsPath, opFamily, spectral, numInputs`, then per input `name, default`.
