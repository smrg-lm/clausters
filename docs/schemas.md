...
Every name is **lowercase and joined**, sclang's selector with its camel hump flattened (`hypotApx` → `hypotapx`, `asInteger` → `asint`). Three of them carried an underscore until 0.10 (`as_int`, `as_float`, `hypot_apx`); those spellings are still **accepted** — a def stored before the rename loads — and nothing emits them.
