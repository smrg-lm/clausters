...
That is how a cut assembled from several takes — or from one take in another order — plays as **one** reader: without it the reader would have to change `bufnum` with sample accuracy, and `bufnum` is initial-rate, so every seam would be a new node and a control message in the middle of playback. A join reads its parts sample by sample, so an interpolating reader crossing a seam sees the next part rather than silence, and the fades are the few milliseconds an editor puts on a cut.
