...
A container that layers editable things — an audio editor's view, a box a hand
has entered — draws several of them on **one rectangle**, so a press is claimed
...
rectangle is claimed by the box and by the curve at once. It is also what an
entered box comes back through.
...
- the **position cursor** (`cursor`, in samples; negative = none) — where a
  ...
  not *seek*; what that means for the sound is the owner's, and on a piece it
...
Both are the group's, so every lane of one piece shows the same two lines, and
