...
| `"cut"` | `start len` (samples) | Ctrl+X over a selection. The host owns no data, so this is a **request**: the owner cuts and answers with what the document now is, and the length change a cut implies is the owner's to decide |
