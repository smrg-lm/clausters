...
| `/gui_headClock id which [transport]` | Draw the playheads of `id` from this counter from now on — `"device"` or `"transport"`, followed on `transport` by which transport's position (an `int32`, 0 unless given, since a server has several). `id` is a **window or a widget**, and it is not optional: an integer may follow the word, so an id that could be missing could not be told from a transport. **The choice is per view**: a widget draws from the counter named on it or on its nearest ancestor, then its window's, then the host's default (`--clock`) — so one window can hold two views that play two timelines, each on its own transport. `device` (the default) is the engine's sample clock, which never stops: what a host watching a live server wants, since its meters, scopes and taps are all on that axis. `transport` is a transport's **position** — it holds while the transport is stopped, jumps wherever `/transport_locate` puts it and wraps at a loop's end, all inside the engine. Natively that number is a row of the shared segment; in a page the host polls `/transport_query` for each transport a visible playhead reads, on the same tick as `/clock_query`. **What it changes for a client**: a view drawing it needs no anchor of its own (`playhead_at` of `0`) and no message per frame, because seeking, looping and pausing are transport commands rather than a line the script keeps in step — which is what lets an editor hand playback to the server and stop computing time. A word the host does not know, or an id no window holds, is reported and ignored, so a typo leaves the line drawing what it was drawing; what was named on a widget goes when the widget is freed. The launch-time spelling of the default is `--clock <device|transport>`, and `--session` implies `transport`. (The verb is `headClock` and not `clock` because both clients already have a `clock` — their application clock — and one word for two things is how a reader loses an afternoon.) |
...
**Which clock the line sweeps by is `/gui_headClock`**, named on the view or on
anything above it, and it decides how much of this a client has to do. On `device` (the default) the counter never stops, so
