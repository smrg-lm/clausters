...
| `"move"` | `kind index x y` (`kind` = `"member"`/`"bus"`; canvas units) | a `graph` box dragged on the patch canvas — one payload per moved box, so the driver owns the geometry |
...
| `graph` | A **patcher** of a bus-wired node graph: member boxes, bus nodes, a wire per connection — a **canvas**: a member/bus with `x`/`y` places freely (absent, the classic stacked columns), boxes drag (`"move"` flows back), a click or a marquee on empty canvas selects, and inside a `scroll` workspace the whole patch pans and zooms | `members` (each `{name, ports[, x, y]}`), `buses` (a name, or `{name, x, y}`), `wires`, `label` |
