...
| `locate` | Puts the transport's cursor under the pointer and emits `"locate"` — or, when the pointer is on a **marker**, at that marker's own time |
| `none` | Nothing |
