...
| `"clip"` | `offset dur start` (timeline units; `start` is the source frame the clip's own time zero reads) | a `clip` moved or trimmed. A move changes the offset; an edge drag **trims** — the placement and the window over the samples move together, which is what makes a trim hide frames instead of compressing them. **A gesture that changed nothing sends nothing**: a click on a clip (which places the cursor, like a click anywhere else on the axis) and a drag that came back to where it began are not edits, so the owner is not asked to apply one and a document takes no entry to undo |
