...
| `patch` | A **directed, typed patcher**: boxes with **inlets on top, outlets on the bottom**, a **cord** per `outlet -> inlet` connection (the buses are not drawn — a cord *is* a bus). A **canvas**: a box with `x`/`y` places freely (absent, boxes auto-stack down a column), boxes drag (`"move"` flows back), a click or a marquee on empty canvas selects, and inside a `scroll` workspace the whole patch pans and zooms | `boxes` (each `{def, inlets, outlets[, x, y]}`; a port is a bare name (audio) or `{name, rate}` (control)), `cords` (a flat `[from_box, outlet, to_box, inlet, ...]` list, indices within each box's inlet/outlet lists), `label` |
