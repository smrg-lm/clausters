...
region, and a linked waveform or roll draws it — but on the stack what is
painted is **the rectangle the hand drew**: each lane shows the part of itself
the sweep covered, so a marquee ending a third of the way down a lane is drawn
a third of the way down it, and a lane the rectangle never reached shows
nothing. It does not snap to lanes — the vertical reach is a value the hand
chose, not a row it landed on — while *which clips it took* is every lane it
touched, since a clip fills its lane's height and a rectangle over part of one
is over the clip. A selection **no sweep drew** — one written through
`sel_start`/`sel_len`, or a def's own — restricts no lanes, and every lane of
the stack draws the whole of its band, which is the only thing a span alone can
honestly mean. The reach is the host's own state, set by the sweep and dropped
by any other write of the selection; it is not a property on the wire, because
a client that never swept has nothing to say about it.
...
| `select` | Sweeps the **marquee**: the container's shared time selection on a timeline, and *what the rectangle covered* wherever the view under it holds anything — a lane's clips, a roll's notes, and, on a plane, a patcher's boxes. One gesture with one rule, so a press is a rectangle of no size and lets go of whatever was held, and the view differs only in what it has to catch and in what else it writes (a timeline's span is the navigation group's; a plane writes nothing but the set). A `plane`'s element claims the press for it, because only that element knows where its own paper ends |
...
The order is the point. `"element select"` is a lane — grab the clip under the
cursor, and if there is none, sweep; `"select"` is a waveform, which has
...
throughout this chapter (`{"drag": "element select", "shift": "pan"}` on a
