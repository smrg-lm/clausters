...
The exception is a `scroll` workspace's **content plane**: its `content_w`/`content_h`, its `view_x`/`view_y` pan and its children's place props are content units, which the host reads as physical pixels because the plane carries a zoom of its own and its pan is written in the pixels the pointer moves. Same reason the heavy views (`waveform`, `spectrogram`) resolve their signal against physical pixels. So: chrome is logical, a navigable plane is its own.
