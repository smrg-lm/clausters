...
- **ASCII in source**: code, comments, doc comments, docstrings and the strings the application prints are plain ASCII in every language — the aside is `--`, and the rest is spelled out too (`...`, `->`, `*`, `-`, `+/-`, `<=`, `sqrt()`, `^2`, `pi`, `degrees`). `tests/ascii.rs` checks every tracked source, configuration and page file and names the file, line and codepoint of anything left. A file whose *subject* is the character — the font's glyph table, the dead-key composition docs, the accent tests, a utf8 test vector — is listed with its reason in that test's `DATA`; a glyph that must reach the screen from ordinary code is written as an escape (`\u23FB`). **The books are not covered**: they are read rendered, so this page keeps its em dashes.
