...
| `src/host/timeline.rs` | The navigation **groups**: the shared window/selection/playhead of linked views and of the multitrack's aligned lanes — plus, beside the group state rather than in it, **which lanes a sweep crossed**: the span is the group's (it is the loop region) but how far down the stack the rectangle reached is the hand's second value, so the band is painted on those lanes and no others, and any write of the selection that is not a sweep drops the range |
