...
| `slider`, `knob`, `number` | `host/controls.rs` (draw + the pure drag math); the shared `Range` payload in `host/widget/props.rs` | the script; value changes emit `/gui_event` (or a binding forwards) |
