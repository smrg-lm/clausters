...
| `src/host/placement.rs` | **One geometry for every box that lives on a time axis.** A note's rectangle in a roll and a clip's rectangle on a lane are the same object with respect to editing and positioning — a span on a **row**, grabbed by one of three `Part`s, snapped to a grid, floored at one sample, bounded by the contents behind it and by the domain around it — and they differ only in what they contain and in what they send. So the arithmetic is here once (`Placement`, `Contents`, `Bounds`, `drag`, the marquee, the rigid block move, the quantize) and both call it. What is unified is the **behaviour, never the storage**: a note stays five numbers in a contiguous `Vec<Note>` and a clip stays a widget in the tree, which is why the accessor is `Placements` over an *index* rather than a common item type — a `Widget` is 13.5x a `Note` and the layout that places one runs per frame and per pointer event. The pixel mappings stay with their renderers (`graphics/pianoroll.rs` maps a pitch to a row, `interact/coords.rs` a cursor to a sample) and hand the numbers here. The **verbs over a selection** are the layer above and are here for the same reason: `Boxes` is `Placements` plus the two questions a verb asks and a drag never does — make another one, take these away — and `split`, `discard` and `rebased` are written over it once, where a clip's `e` and a note's `e` had been two implementations that had already drifted (one refusing out loud, the other returning silence). What stays with the holder is the identity of a new box: a clip is minted with a fresh name, a note keeps the pitch of the one it was cut out of |
