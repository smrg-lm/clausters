...
The portable, always-on part is the **CPU meter**; the default `rtprio` feature adds the Linux-specific scheduling around it. Together they answer "how many voices fit before the audio breaks", reliably (see `examples/stress.rs` for the harness that asks it).
...
- **The load table** (`server::meters`, always on, portable): the CPU meter says whether the audio thread is keeping up; the table says *where the server's time goes*, which is what a session that has grown heavy actually asks. One slot per **role** — `audio` (the block, fed by the measurement the CPU meter already takes, so it costs no second clock read), `dsp` *i* (one worker thread: the stages it took off the conductor, bracketed per stage), `net` (a serving turn, never the blocking recv), `nrt` (a job) and `faust` (a compilation) — each a pair of relaxed atomics holding cumulative nanoseconds and a call count. `/server_load.reply` reports them in seconds since boot, and the client differences two replies into a window of its own, which is what lets several clients poll at once (the peak in `/server_status` cannot: reading it resets it). Two things the numbers are not: a **thread** (a role is the work, so a build that runs one elsewhere reports the same rows) and a **per cent of a core** (a worker spinning for its next stage is burning a core and is idle here — `top -H`/`perf` answer that question). Inert on wasm32, which has no monotonic clock.
...
| `crates/clausters-net` | the stream carriers, shared with the GUI host: TCP (on by default; length-prefixed framing) and WebSocket, their acceptor and connection threads, the client ceiling (`ClientSlots`), the `Hub` the network thread drains and the `Waker` that ends its recv |
...
The **boot-flag** column marks what is configurable at server start (S7): the four pool sizes plus the buses and the hardware I/O channels (`--outputs`/`--inputs`). Every one sizes a slab or `Vec` built **once at startup** — fixed at runtime, never at compile time — fed by the same config-file → flag precedence as the other options (`[server]` keys `max_nodes`, `max_buffers`, `max_graph_children`, `max_ugen_inputs`, `outputs`, `inputs`). One carries a compile-time hard ceiling the flag clamps to, because it sizes a fixed-width structure the audio thread relies on: UGen inputs at 32 (the per-UGen input list is a stack array in `synthdef::instance`). The bus counts had one too — audio buses at 128, because a node's usage mask was a `u128` — and it is gone: a node's exact usage is a bit vector built on the network thread (`dsp::BusUsage`) and what the engine packs parallel stages from is a conservative summary of it folded into 128 lanes (`dsp::StageMask`), where two buses landing in one lane cost parallelism and never correctness. Both counts must be powers of two, and what a bus costs is memory plus the per-block clear (`tests/bus_scale.rs` measures it). A client discovers the live values with `/server_query` (see `schemas.md`). `tests/capacity.rs` overflows the configurable slabs at both the default and a small custom size.
...
The engine publishes its **sample clock** — samples processed since start — as an `AtomicU64` (`EngineHandle::current_samples`). The conversion from an OSC NTP timetag to an absolute sample position happens on the **network thread**; the engine itself never looks at wall time. It goes through the **device epoch** (`server::device_epoch`): the audio callback stamps, once per callback, the frame it is about to hand the device with the system clock, and publishes the lower edge of `stamp − frame / rate` — the Unix instant sample 0 falls on — so a timetag `T` lands on sample `(T − epoch) × rate` whenever its packet was handled. The counter alone would not do: it moves once per callback, and pairing it with the wall clock *now* is off by how far into the callback the packet arrived, a different amount for every bundle, which is a device buffer of jitter on timing the timetags carry exactly. A host that drives the engine itself (headless, NRT, wasm) publishes no epoch; its time *is* the counter, so it keeps delta × rate from the published count. For the same reason `backend::start` does not return until the device has called back twice: the socket is bound after it, so a server that answers is one whose stream is sounding and whose epoch has settled, and the first bundle of a client that booted it lands on the same line as every later one. A timed bundle becomes `Cmd::Schedule { time, cmds }` in a pre-allocated, stably-sorted queue; when its sample falls inside a block, the block is genuinely **split** at that frame — `ProcessCtx` carries `offset` + `frames`, and every UGen/synth processes the sub-range — unlike scsynth, which quantizes to block boundaries and needs `OffsetOut`. The spent `Vec` shell of an executed bundle returns through the garbage FIFO (`Garbage::SpentBundle`) to be freed on the network side.
...
**A clock is not a position, and the third quantity is the one an editor wants.** Both counters above are monotonic — that is what makes them clocks, and what the transport queue needs, since "due" only means anything on an axis that cannot jump. Neither of them says where the transport *is*: `/transport_locate` must be able to move that, and moving a clock is not a thing. So `TransportPosition` sits beside the two in `server::clock_axis`, a type again for the same reason they are, and it is **anchored rather than accumulated** — `PositionAnchor` holds the position a locate put it at and the transport sample that locate landed on, so a read is one add, a seek is one store, and the per-sample path costs nothing. A loop is the same store: the engine cuts its block at the wrap exactly as it cuts at a scheduled bundle, which keeps the position linear inside every slice, so the `TransportPos` UGen a buffer reader follows just ramps and never has to know a loop exists. A wrap landing on the block boundary belongs to *that* block, because the position published at a block's end is what the next block's first sample plays.

**There are several transports, and all of the above is per transport.** `Engine::transports` is a table sized at boot (`--transports`, `Limits::transports`) and never grown: each `TransportState` holds its own rolling state, `frozen_total`, position anchor, loop, end mark, governed group and queue, and each has its own clock — the device clock minus what *it* has spent stopped — so a stop on one moves nothing about another. Every transport command carries the index. The block-cut loop takes the union of all of them: the earliest entry of any rolling transport's queue, the earliest loop wrap or end mark of any of them, and the device queue, each resolved to device samples; ties between transports go to the lower id. **Which transport a node reads** is the tree's: binding a group tags its slot with the transport (`NodeTree::set_governing`), the engine writes each transport's `TransportCtx` for the slice into a table on the tree (`set_transport_view`), and the walk swaps `ProcessCtx::transport` for the tagged row on entering a tagged group, so a node reads the transport of the nearest bound group above it and everything else reads transport 0's. A group can also be bound to **follow** a transport (`NodeTree::set_following`): it reads it and is neither frozen by it nor routed to its queue, since the routing walk skips a tag that does not govern. A scheduled bundle is routed the same way (`NodeTree::governing`), so it waits on the queue of the transport governing its first governed target. **A stop can be a phase rather than a sample** (`/transport_fade`): with a ramp set, `TransportState::stop` leaves the transport rolling and records a `Stopping` edge a ramp's length ahead, the declick level (`TransportState::fade`, a straight `Ramp` over device samples published per slice as `TransportCtx::fade`/`fade_step`, read by `TransportFade`) falls to zero across it, and the freeze is that edge of the block-cut loop — so everything the freeze does happens there, on a sample, as it does for the end mark. The end mark's own edge becomes the ramp's length before it (`Edge::EndRamp`), which starts a stopping phase that ends on the mark.

Each transport keeps a queue of its own beside the device one, whose entries live on its axis; the block is cut by whichever queue falls due first, compared in device samples, and a stopped transport can never reach its own queue — **the pause freezes the queue by never letting it fall due, without rewriting anything in it**. Ties go to the device queue: a fixed preference, since cross-queue enqueue order is not recoverable at fire time (a transport entry's device time is not fixed when it is enqueued), and device-first makes an empty transport queue indistinguishable from a single queue over the device axis.
...
These two are the one place a UGen needs **synth-private** state that `ProcessCtx` (global, shared by the parallel scheduler) cannot carry, so `UGenSynth::process` handles them inline (matching on `def.ugens[i].kind`) instead of through the `UGen` trait; `src/dsp/local.rs` holds only placeholder structs. They touch no global bus, so `osc::graph::ugen_usage` gives them an empty `BusUsage` and feedback synths still parallelize safely (private state, nothing shared). The channel index must be a constant (so the buffer is sized at compile time, `SynthDef::num_locals`).
...
**The range maps take the same shape one arity out.** `RangeMapUGen` (`src/dsp/rangemap.rs`) is a third `op_family` row, carrying a `clausters_core::warp::MapOp` by name over the inputs `in, in_lo, in_hi, out_lo, out_hi[, curve]` plus a static `clip`. It computes nothing of its own: the formula is `warp::Map`, the same one the client's value functions run, which is what keeps a mapped signal and a mapped control equal. Its one bit of engineering is that a map's *bounds* decide part of the arithmetic — `ln(hi/lo)`, `exp(curve)`, the bend's terms — so a block whose bounds are all constants builds one `Map` and every sample pays only `Map::at`; a bound that is a signal falls back to preparing per sample, which is what modulating a range costs.
...
`TempoClock` keeps musical time (beats, on its own thread, the music plays on it);
...
for the same beat forever and is meaningful for a structure that has never been
...
(`form.render.flatten`, which takes the tree's tempo map), because a timeline is
...
tempo change convertible — the clock never asks, but a view of the structure asks
...
server transport *declares the map affine* (`TempoClock.join_transport`
replaces the map with that single segment). A map whose tempo changes aligns
...
session's **source table** says that, so opening a multitrack into a running system
...
instead of reaching a multitrack. The multitrack editor is still the Python
client's alone.
...
- **The four layers.** *Sources* (samples, never overwritten) / *the document*
...
- **A node id names a *placement*.** A clip is a window onto samples, so what
...
**Playing a multitrack is one object, and it sends nothing.**
`clausters_editing::playback::MultitrackPlayback` holds a multitrack's instance (the
difference between what a server holds and what the multitrack says), its applier
(every operation as the messages it is, allocating from the endpoint's
`IdSpaces`) and its transport: the tempo a multitrack that states none is read at
(`DEFAULT_BPM`), the sample a beat is when the transport is located, what play,
pause, stop and cue send, and the zeroed meters of a paused multitrack. Every verb
...
their own `Playback`. So a session edited with no script behind it and a multitrack
...
(`clausters_apps::multitrack::window`): a time ruler above the multitrack on its
own axis, the `multitrack` widget, and — when the multitrack can be heard —
...
### The editor's processes: who owns the samples, who holds the devices

An editor that owns its samples through a real-time server holds an audio device it does not need, cannot be restarted without taking the samples with it, and pays the whole real-time surface to run three verbs. So the standalone editor is **three roles**, and the split is by what each one holds:

| Role | Holds | Where it lives |
|---|---|---|
| The **editor** | the document, and the gestures on it | `clausters-gui --session` |
| The **on-demand session** | the **samples**: the segment and every take in it | in the editor's process (`ClaustersSession`, an `NrtSession` on its own thread) |
| The **player** | the machine's **input and output** | another process: `clausters --shm <path>` |

What decides the split is the one thing a segment cannot have twice. Its rings are **SPSC** — one pair, one drainer — so the first server on a segment claims the command plane (a pid in the header) and owns the samples; any later one attaches to the data plane, maps what the owner published, and serves its own clients over its own sockets. That claim is why `--shm` **attaches** to a segment that exists instead of truncating it, and a claim whose pid no longer answers is stale and is taken over, so killing a server is recoverable rather than terminal.
...
- **Attaching restores the samples and not the routing.** Ports and the connections a person made to them live with the process, so a restart is recovery. It is only cheap if the ports come back under the same name, which is what `--client-name` (and `--device` under JACK) is for.

The editor's own path follows from it: allocation, the editing verbs and `/buffer_render` go to the session that owns the samples; playing, sounding a key, the transport and the bus taps go to the player (`Host::server` and `Host::player`). Takes reach the player as **`/buffer_attach`** and never as samples. And with the take mapped, the editor draws it by reading the region and edits it by **storing into the cells** — no `/buffer_query`, no chunked `/buffer_getRange`, no blob out and no reconciliation.

**The picture reads the samples rather than a copy of it.** A view over a mapped take holds one source per channel (`peaks::Source`, the core's own door) and its peak pyramid is summarized over the region, so opening a ten-minute take allocates the summary and a bounded read window — not the take. Two things follow. A write is only ever a **summary** update over the span that moved, because the samples were already stored where they live; and a zoom finer than a bucket reads the cells themselves, so an edit by anybody is in the picture with nothing told to it. What still has to be told is the overview: this host sends `/buffer_touch` after its own write (the span, so a Python or web client holding a picture of the same take learns it changed) and **consumes `/buffer_touched`** for everybody else's, re-summarizing that span in every view of that buffer. A client that cannot map — a page above all — holds its own samples and has to fetch the span back instead, which is the same split every bulk path has.

**A host that cannot map keeps two routes to a picture, and the size is what chooses.** They are cheap in opposite cases, which is why both stay. A **short** buffer is downloaded whole (`/buffer_getRange`, end to end): one conversation, and then every zoom is answered out of the samples in hand with no further round trip ever. A **long** one is drawn from its summary — `/buffer_peaks` for a take standing still, the `/buffer_stream` reports for one being recorded into, the same blob folded the same way — with the run under the eye read back as the eye moves, because a ten-minute stereo take is 230 MB and no zoom makes that downloadable. The threshold is roughly **five seconds of stereo**, and the decision is made once, where the shape is first known (`fetch::BufferFetches::whole`, on `/buffer_query.reply`). It is a count of round trips as much as of bytes: a whole download is one request per chunk, each gated by a frame, and the chunk is sized so several replies fit the carrier's ring rather than being dropped by it. It used to be the `fills` prop — *is this being recorded?* — which is a question about the samples' state standing in for one about cost: two views of the same finished take, one opened while it recorded and one after, took different routes and behaved differently under the same hand. `fills` now says only what it means, that the samples are being written and are therefore not there to fetch.

**And a recording is drawn as it fills**, which is the one write nobody announces: a `RecordBuf` filling a take says nothing on the wire, correctly. What it publishes instead is a **write frontier** in the buffer's directory row — how far the samples now goes — and the host compares it against the frontier each view was last summarized at, re-reading only the frames between. A window redraws only if something grew, and only a take whose frontier is past zero and short of its end keeps the window awake, so a session of loaded takes still sleeps.

**The picture grows at the frame, and what makes that affordable is that a step costs the step.** Re-reading a summary used to cost a *copy* of the whole take — the pyramid is shared with whatever slot is drawing it, so it could not be written in place — and a copy does not shrink with the block, which is why the block had to grow instead (one second, and a take that jumped a second at a time). The holder **gives the samples back before the write** (`WaveformView::release_data`, called by the leg that follows the frontier) and takes it again on the next fill, which a repaint runs first; in between the element is the sole owner and `Arc::make_mut` hands it `&mut` with no copy at all. Summarizing a span touches the buckets over it and their parents, so a step is about five microseconds a channel whatever the take's length, and following at the frame costs a millisecond a second with eight takes recording. The rest of the shape stays: every channel of a take is refreshed together (the frontier is the buffer's), and the tick is **one for every view** rather than a wait each view keeps, so any number of takes repaint a window once. `--follow-block` is still there for the case where even that is too much, defaulting to zero (the frame). Over the wire the same thing is `/buffer_stream`, which carries the *summary* of what was written (min, max and mean square per bucket) rather than the audio, at about a hundredth of the bandwidth.

**And the host takes that wire when it has no mapping to read**, which is the page's ordinary case and a native host's whenever the server shares no segment. The rule is the element's: a view whose samples are its **own copy** and whose client set `fills` asks to be told (`Element::stream_want`), the host keeps **one** subscription covering every such view of every window (`Host::sync_buffer_streams` — the server keeps one per client, so a second call would cancel the first), and each report is folded into the pyramid the view already holds (`Element::write_buckets` → `WaveformData::write_buckets` → `peaks::Pyramid::write_buckets`, the core door both clients bind). Nothing is measured on the receiving side, so the picture a page grows is the picture the samples would have built — asserted across the language seam by the web client's parity vectors. What it is not is the samples — so a view zoomed finer than its summary asks for **the span it is showing**, and a page resolves to the sample exactly where a mapping host reads the cells. **In one of two shapes, and the zoom is what chooses**, because what a picture needs is one min/max pair per pixel column and there are two ways to be sent it. Between the summary's bucket and about thirty-two samples a pixel the answer is a **finer grid** over that span (`/buffer_peaks` at a bucket sized so a column holds two of them, `Detail` beside the channel's pyramid, `Element::set_detail`): a few kilobytes, one reply, and no samples move at all. Finer than that the answer is the **samples** (`/buffer_getRange` over that run, `Samples::Window` under the pyramid, `Element::set_window`), which is where a grid would carry three floats to describe a handful and where the trace becomes the polyline through the samples anyway. The grid is replaced rather than extended, like the window beside it — one grid at a time per view, because what it is for is where the eye is — and its bucket is a power of two, so zooming out is free and zooming in asks again only after a factor of two. That holds **while the take is still recording**: the write frontier is a ceiling on the span and not a refusal of it, since the frames the report counts were written before it was measured and a recorder writes forward and does not come back, while past the frontier the buffer still holds the zeros it was allocated with and a run over them would claim measured silence over audio that has not arrived. The draw pass is what notices, because it is the only place that knows the zoom, the span and the frontier at once (`frame::owed`, whose `Owed` says which of the two shapes would settle it); the leg turns that note into a request, one download per buffer at a time and one grid per view, and nothing is asked while the summary is the right answer.
...
| Path | Contents |
|---|---|
| `src/host/mod.rs` and its concern modules | The protocol core, one concern per file: `mod.rs` holds the `Host`, `HostEffect` and the `/gui_*` vocabulary; `wire.rs` the dispatch (`/gui_def`/`/gui_set`/`/gui_free`/`/gui_query`/`/gui_bind`) and the one way arguments are read; `define.rs` a definition, a free and what a missing widget leaves behind; `set.rs` a live set; `answer.rs` a gesture answered by the owner of the document; `takes.rs` the samples a widget draws and the one walk over the pictures of a buffer; `clocks.rs` the counter a playhead reads; `replies.rs` the audio server's buffer replies, read once for both fronts through the `Front` trait; `winit_keys.rs` the key both shells read |
...
| `src/host/elements/multitrack/` | **The one element that is an application**, and the only leaf with a directory of its own for that reason rather than for a product's sake (`signal/`'s children are the trait's own questions). It was 4650 lines holding a model, its props, its picture, its hit-testing, its gestures, its verbs and its tests, and it is split the way `interact` and `gestures` are: by the **question a reader arrives with**, since that is what a reader has one of at a time. `mod.rs` is the type, what it holds, and the *table* of what it answers — a trait method longer than a few lines delegates to an inherent one in the module that holds its question; `props` is the wire's lists becoming the widget's own and every payload a gesture leaves (one module, because it is one contract read from either end); `rows` is the vertical model (where a row is, which row a `y` is in, how tall each is once a reader has zoomed one) and the time axis beside it; `draw` is the composition; `hand` is every phase of a gesture and the state a press claims; `verbs` is what the multitrack can be asked to do and what it refuses with. What is *general* to an application over the document is deliberately not hoisted out of it yet: there is one application, and a shared module with one user is a guess — see `clients/gui/PLAN.md` |
| `src/host/elements/` | The leaves that have **moved behind the trait**: one type per widget in its own file with its tests beside it, and one row in `elements::builtin` — the table the schema consults *before* the shared registry, so a built-in name can never be shadowed by a registration. A leaf here is not an arm anywhere: `set` is its apply, `natural` its size, `draw` its frame pass, `needs` its whole declaration to the tree collectors, `hit_area` the shape it answers the pointer on (the placement rectangle by default; a disc or an ellipse for one drawn smaller or rounder than its cell, filtered once by the gesture machine rather than guarded per leaf). What is **not** on that trait is as deliberate: a picture of samples owes nineteen answers nothing else in the catalog has (what shape they are, which buffer they came from, what a span is worth, how a write lands, what the view wants next), an element on the **shared time axis** eleven more (how far its content reaches, what it reserves beside the axis, how many rows it stacks, the navigation chrome it carries and is written through), one with a **measured axis of its own** four, and one whose picture is a **texture** five — so those are **facets**: `Samples`, `OnAxis`, `Measured` and `Slotted`, reached through `Element::samples`/`on_axis`/`measured`/`slotted` and through `WidgetKind::as_samples` and its kin at the call sites, each answering `None` by default. A capability is then *stated* rather than declined thirty-nine times (the trait is **41 methods where it was 75**), and the families the audio and score editors grow add to a facet instead of to every leaf's interface. What stayed is what a control *is*: the hand — press, drag, release, the keys, the focus — is not a facet, because the median element here is a control and a facet is worth its accessor only where the capability is the exception |
| `src/host/layers.rs` | **Which of a container's layered contents a hand is editing** — the one rule that decides between claimants over the same pixels: one layer is active at a time, and it is the only one that acts or offers an affordance. The stack is read off whatever children fill a body role, so it names no widget type and grows with the container; the active layer is a field of the `Widget` node rather than of a `Clip`, because a clip is the first container that layers editable things and deliberately not the last (an audio editor's view is the same picture). Selecting one is an operation of its own (`Selection`) with three callers and no privileged one: the pointer rule (`under_pointer`), `/gui_set layer`, and whatever asks next. An element contributes one answer, `Element::layer_hit` — whether the pointer is on its **own samples**, never on the rectangle it shares with its container, which is what leaves a clip's background and its grips to the clip. `visible`/`hidden` is the same question about *drawing*, and the two meet in one rule: what is not drawn is not edited either |
| `src/host/structures/` | **The structures a hand edits, and the verbs over them** — apart from what draws them. `boxes.rs` is one geometry for every box that lives on a time axis: a note's rectangle in a roll and a clip's on a lane are the same object with respect to editing and positioning (a span on a **row**, grabbed by one of three `Part`s, snapped, floored at one sample, bounded by its contents and by its domain), so the arithmetic is written once and both call it. What is unified is the **behaviour, never the storage** — a note stays five numbers in a contiguous `Vec<Note>` and a clip stays a widget in the tree, which is why the accessor is `Placements` over an *index* rather than a common item type (a `Widget` is 13.5x a `Note`, and the layout that places one runs per frame and per pointer event). The **verbs over a selection** are the layer above it: `Holder` is `Placements` plus the two questions a verb asks and a drag never does — make another one, take these away — and `split`, `discard` and `rebased` are written over it once, where a clip's `e` and a note's `e` had been two implementations that had already drifted. Beside it, one module per structure: `notes.rs` (a note and a roll's list verbs), `clips.rs` (a lane, a box on it, a curve over either, and the payload each list reports as), `points.rs` (a break-point, its value axis and the edits that move one). The boundary is stated negatively, which is what makes the layer worth separating: **nothing here names a `Rect`, a `View`, a `Draw` or a `Metrics`** — a module that took one would be a drawing again. It exists because three applications are being built over one document and the *structures* are the part they share, where the pictures are not: they were written under `graphics/`, beside whoever drew them first, so the module whose name says drawing held the model, its edits and its picture |
...
| `src/host/status.rs`, `src/host/diag.rs` | **What the host says, and to whom.** `status.rs` is the band along a window's bottom edge — the hand's own history: every event the host emits (at the one place an event is stamped), every value that leaves by a binding instead, and every `reason` an acknowledgement carries, collapsed by widget-and-verb so a drag is one line rather than four hundred. Nothing of it crosses the wire: the host already knows what it did, so asking a client to send it back would be telling the host something the host said first. `diag.rs` is the other audience — what the host says about its own working — and it is one door because `tracing` was only half of one: the native binary installs a subscriber and a page installs nothing, so a hundred diagnostics printed in a window and none of them in a tab, which is the kind of native/browser difference the crate treats as a defect. `warn!`/`info!` survive a release; `debug!` and `note!` are compiled out of one, arguments included, and `note!` also writes its line onto the window's status bar, where somebody debugging is already looking |
...
| `src/host/gestures/` | The one press → drag → release → wheel state machine **both fronts drive**: it mutates the `Host` through the `interact` doors and returns `GestureEffect`s (emit/redraw/release-pointer) for the front's own sinks, so every editing gesture behaves identically on either platform by construction. A press runs the **containers' plans** over the chain, innermost first (pan / select / locate / hand it to the element, each step free to decline and pass the press outward), so the axis' gestures exist once for the five timeline views instead of once per kind; the element step is where a widget's own behaviour lives. The **marquee** is one of the container's, everywhere: the machine holds the anchor and the frame draws the rectangle, and the only part an element answers is what fell inside it (`Element::select_in`) — a press being a rectangle of no size, which is what makes a click let go on every view by one rule. It was written three times before that (a patcher's boxes, a roll's notes, a lane's clips) and the three disagreed about the modifier, about whether a press clears and about what counted as inside. **The click is the machine's for the same reason**: where a press landed on the time axis is read once, at the press, and a release that never left the hit slop puts the window's one cursor there — whatever took the press, so a clip, a note, a roll's grid, empty lane space and a ruler all locate by one rule and the content never moves the cursor (the steps that answered for the pressed pixel themselves — `locate`, `marker`, `sample`, `draw` — cancel it). While the transport is running that same click re-anchors the sweep, because the drawn cursor and the playing one are one thing. **Two steps, because they are two selections**: `marquee` selects the objects a rectangle covered (a multitrack's clips, a patcher's boxes) and leaves no picture behind it, while `select` sweeps a *time range* — a span the navigation group keeps, drawn as a band, looped by the transport. A multitrack's plain drag is the first; the second is a real and separate selection over the same lanes, asked for by name. Split by **phase**, because that is how a gesture is read and changed — a variant's press, its drag and its release are three arms in three matches: `mod.rs` is the `Drag` enum and the state, `press.rs` (the plans, then the element — where every drag is opened), `drag.rs` (what a held drag moves, what its release delivers, the edge-scroll tick), `wheel.rs` (the phase that opens no drag). Beside them: `effects.rs` (what a gesture delivers — the one bound-vs-event decision), `nav.rs` (what it reads and moves), `keys.rs` (the keyboard half: Tab to the ring, then the focused element, then the element under the **cursor** — which is where a view's block operations are addressed), `focus.rs` (the ring itself, and the one door that moves the focus) |
| `src/host/elements/signal/` | The **signal element**: one element for every view of a signal — a *layer stack* over a *presentation* (the trace, a magnitude spectrum, the time-frequency texture, the phase of a stereo pair) of a *source* (random-access samples, or a bus read forward-only) with the *capabilities* the view offers over it, plus the table mapping each of the six wire names onto a point of that product. The one leaf that needs a directory rather than a file, and its children are the trait's own questions: what a `/gui_set` means to it (`apply`), what it loads (`bulk`), what it hands its GPU slot (`slot`), what it accumulates between ticks (`live`), the frequency axis it alone measures (`freq`), the band it reserves left of a shared axis (`chrome`), what it is inside a clip (`body`) |
| `src/host/graphics/` | **The drawings**: what a visual thing looks like, and where a click on that drawing lands — pure over a `Draw` (the batch plus the two role tables, `host/paint.rs`), unit-tested without a window. What each thing *is* and what a hand does to a list of them is `host/structures/` beside it, and the two meet at the numbers: `pianoroll` maps a pitch to a row and a cursor to a note, `bpf` maps a value to a height, `multitrack` lays the rows of a stack out. The boundary is stated negatively, which is what makes it worth separating: nothing here names the `Host`, the `Element` trait, a props map, OSC or a GPU device, and the dependency runs one way — an element reads its drawing, never the reverse. A parallel tree rather than a file beside each element because the cardinality does not match: a drawing may serve several elements (`controls` draws eight), an element *and* a container's body (`pianoroll` draws the notes of both the `notes` leaf and the multitrack `clip`'s roll body, so the two never disagree), or no element at all (`track` is a container, and its lane is still drawn here) |
...
| `src/host/instance.rs` | **What the multitrack is playing through.** A standalone host plays a multitrack the way every other endpoint does: `nodes::plan` says what it needs, `clausters_editing::instance::Instance` answers the difference as `Op`s, and `clausters_editing::apply::Applier` turns them into **steps** — messages, and the replies some of them wait for (a buffer's `/done` before its fill, a `/server_sync` barrier). This module is the socket: it pushes the steps into the crate's `clausters_editing::run::Runner`, sends what it says may go out — each to its server, the session that holds the samples or the player that sounds — and hands it every reply `Host::on_server_reply` brings from either front, with the leg it came in on. The ids are the host's spaces (`ids.rs`). The transport is the crate's, as it is every endpoint's: the playback makes the transport's group at the top, which **follows** transport 0, makes the multitrack's graph inside it and binds the graph's tracks' group as the one the transport **governs** — so a stop freezes the tracks and the master around them, its meter and its way out, go on running and fade across the stop. The take monitor is `play.rs`'s, on a transport of its own. It also makes a source an edit mints (a join) before the picture is redrawn: session table, take table, and a `/buffer_stitch` in the session that the player's `/buffer_attach` waits behind. Beside `play.rs` and not under `document/`, because what it holds is the server's and nodes are not the structure they play |
| `src/host/ids.rs` | **The node ids, buses and buffers the host allocates** on the server it plays through, by the one client policy (`clausters_core::ids::IdSpaces`): sized by `ServerShape::DEFAULT` until the `/server_query.reply` reshapes it, and sliced by the share a launcher gives (`--id-share`, the page bridge's `id_share`) when a script allocates on the same server. `Host::on_server_reply` is the door both fronts call for every reply: a `/node_end` gives a node id back, the server's shape reshapes the spaces, and the multitrack's waiting step is offered the rest. `Host::on_link_attached` registers for node ends and asks the shape, on every link, whoever launched the server; the monitor's defs go with its first play, sent by its playback |
| `src/host/play.rs` | The **take monitor**, for a window that draws samples with no application behind it: the audio editor's playback (`clausters_editing::audio_playback`), held by the host on a transport of its own (`MONITOR_TRANSPORT`, 2), so a take pane never moves a multitrack's position. Every take it has played stays made and paused, the one played last is the focus, and the output beside them meters and declicks. The space bar plays from the position cursor to the end of the take or the selection — the transport's end mark, which the engine stops on and locates back from — or, with `L`'s loop on, repeats; a second press stops it back at the position cursor, and placing the cursor while nothing plays cues the transport there, so the play cursor stands on it. A window that says `plays` is its owner's, and the space bar is then the window's `play` verb with the loop beside it. A pass the engine ended is told from a stop this host sent by counting the stops it sent (`play::Follow`) |
...
| `src/host/graphics/selection.rs` | **The sweep**: what a swept selection looks like, wherever a hand draws one. One routine for the five views that let a hand sweep — the lane, the roll, the waveform, the spectrogram, the patcher's canvas — told only what the view's second axis measures (`Vertical`), which is the one question a sweep asks that a view answers differently: a full-height stripe where nothing restricts it, the rectangle the hand cut where something does. The **marquee** — the objects a rectangle covered — is drawn from the machine's own in-flight rectangle (`Grab::Marquee`) through `draw_rect` here, and is gone the moment the hand lets go. The edges follow from the same answer rather than a flag. It exists because the five had written the drawing themselves and had drifted: two arithmetics for the band's edges, a roll that swept a pitch band and drew a full-height stripe, a lane that drew nothing at all |
...
| `src/host/timeline.rs` | The navigation **groups**: the shared window/selection/playhead of linked views and of the multitrack's aligned lanes — it is also where a lane's span is left **unclamped**: a far edge belongs to data, and a lane holds none |
...
- **Edit-back is a payload, not an address.** A view that writes data back emits
  `/gui_event <id> <tag> <flat values…>` — `"points"` for a curve, `"clips"` for a
...
| `plane` (with `boxes`) | an **element**: `host/elements/patch.rs` (its two drags — a cord and a box move — plus its answer to what a marquee caught, which is the one part of a sweep an element owns: the gesture itself is the machine's `Drag::Marquee`, claimed here because only the canvas knows where its own paper ends) over `host/graphics/patch.rs` (box/port geometry, the cords, hit-testing and the layered `solve` layout — pure), one implementation for both levels; the rate vocabulary is `clausters_core::patch`'s | the patch model a client compiled or decoded: a GraphDef's members and cord→bus wiring at level 1, a SynthDef/FaustDef's UGen graph at level 2; drawing a cord emits `"wire"`, dragging a box `"move"` |
