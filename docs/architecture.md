...
A UGen can ask for its node to be freed once it finishes: the `UGen` trait has a `done(&self) -> DoneAction` hook, checked right after `process`. `EnvGen` is the first user — it returns its `doneAction` (`FreeSelf`, `PauseSelf`, `FreeGroup`, or `None`) when its last segment ends. The synth aggregates its UGens' actions (`UGenSynth::done_action` takes the strongest), and the tree walk records any node whose action is `FreeSelf` into a **lock-free finished-node queue** (`NodeTree`'s `done_nodes`/`done_count`, index reservation by `fetch_add` so the parallel workers can push concurrently). The queue is drained **once per block, after the whole tree has run** (`Engine::process_block`): each id is freed through the ordinary `NodeTree::free` path, so the boxed synth leaves on the garbage FIFO exactly like an `/n_free` — never dropped on the audio thread. Freeing after the full walk (not mid-traversal) keeps the tree stable while it is being read, and makes a re-queue from a mid-block schedule split harmless (freeing an already-gone id is a no-op). The queue's atomics only need to be race-free for the concurrent reservation; visibility to the drain is provided by the worker pool's join. `PauseSelf` and `FreeGroup` are decoded but not yet acted on.
