...
3. **All incoming OSC bytes decode through `osc::decode_packet`.** Whatever the transport — UDP datagrams and IPC ring contents are equally untrusted. rosc 0.10.1 over-reads the padding of blobs whose length is a multiple of 4 (top-level: `Err(Eof)`; inside a bundle: the element is **silently dropped**). `decode_packet` splits bundles by hand, recursively. Do not reintroduce `rosc::decoder::decode_udp` without verifying both fixes upstream (see `CLAUDE.md`).
