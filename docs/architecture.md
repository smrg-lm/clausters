...
- **NRT thread** (`server::nrt`): all `/buffer_*` work — allocation, file reading (WAV via hound, compressed/other formats via symphonia), WAV writing via hound, zeroing. One queue, so commands on the same buffer complete in submission order. Produces immutable buffers the network thread installs with `Cmd::SetBuffer`.
...
- **Buffers**: `Arc<Buffer>`, **immutable once installed**. The NRT thread builds them, `Cmd::SetBuffer` swaps them into the engine pool, the replaced `Arc` returns as `Garbage::FreedBuffer`. "Mutating" commands (`/buffer_zero`, `/buffer_read` into an existing buffer, `/buffer_gen` filling a wavetable) build a replacement instead of touching shared memory. The network thread keeps a mirror for `/buffer_query`/`/buffer_write` and for validation.
...
| Graphic unit (a clip: length = duration) | the placed rectangle, and its bodies as the child elements they are | `clients/gui/src/host/track.rs` |
...
**The patcher draws two levels through one widget.** `src/host/patch.rs` is one
...
`host::patch::solve` is a small **layered (Sugiyama-style)** layout — layer each
...
extent but never below the viewport (`host::layout::scroll_content` reads
`patch::natural_size`), and the layout **centres** the block in that area — so a
...
| `text` | `host/elements/text.rs` (the field, its caret and its `key`), drawn through `host/controls.rs`; the caret model in `host/textedit.rs` | the GuiDef, `/gui_set`, the keyboard |
...
| `menu` | an **element**: `host/elements/menu.rs` over the drawing in `host/controls.rs`; its open list is its own state, *declared* as an overlay so the frame draws it last and the press routes to it first | idem |
...
| `signal` (`view: "spectrum"`, a `bus`) | idem; `host/spectrum.rs` keeps the across-frame smoothing, and the per-frame curve (window, FFT, decibel scaling) is `clausters-core::spectrum`'s. With `navigable` it is the one navigable axis that is **not** the window's time: its x measures frequency, so it is in no group and carries its own normalized window (`EditorProps::x_view`), zoomed and panned in `host/gestures/nav.rs` beside the vertical ones and drawn by remapping the columns and `ruler::hz_ticks_h` through it | `channels` adjacent audio buses |
| `nodes` | an **element**: `host/elements/nodes.rs` over the drawing in `host/nodetree.rs`; the group it declares is what the client leg queries | `/group_queryTree` over the client leg + node notifications, filed in the world |
| `canvas` | an **element**: `host/elements/canvas.rs`, which *claims* the shader slot rather than drawing — the pipeline and the `CanvasView` are `host/canvas.rs`'s, one per window | `/gui_set` params and/or control buses, resolved from the world each frame |
| `signal` (`view: "trace"`, navigable) | idem; data + GPU renderer in `src/waveform.rs` over `src/viewport.rs`; chrome via `host/frame/` | `cache`/`path` (mapped or fetched), a server `buffer`, or inline `data`/`blob` — `host/{bulk,mapfile,fetch}.rs` |
...
| `curve` | an **element**: `host/elements/curve.rs` over the model and drawing in `host/bpf.rs`; the shape math is `clausters-core`'s (what `EnvGen` plays). One element in two placements — standing on its own it draws a framed field over its own domain, and *declaring* the curve body role it fills a `clip`, drawn bare against the container's axis (`bpf::Axes` is the one mapping either way) | the script's `points`; edits emit `"points"` from whichever placement drew it |
