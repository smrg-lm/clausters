...
| `src/host/timeline.rs` | The navigation **groups**: the shared window/selection/playhead of linked views and of the multitrack's aligned lanes — plus, beside the group state rather than in it, **how far down the stack a sweep reached**: the span is the group's (it is the loop region) but the rectangle's vertical half is the hand's second value, kept per crossed lane as the fractions of it the sweep covered — so what is drawn is the rectangle rather than the lanes it touched, and any write of the selection that is not a sweep drops the range. It is also where a lane's span is left **unclamped**: a far edge belongs to data, and a lane holds none |
