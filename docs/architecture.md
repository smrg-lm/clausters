...
The whole two-queue path sits behind the `transport` Cargo feature, on by default. Built without it, `process_block` contains the loop it contained before the transport existed — the original walk is kept byte-identical inside `#[cfg(not(feature = "transport"))]` rather than factored, so "an opted-out build runs the old code" is verifiable by reading rather than by trust. That duplication is deliberate.
