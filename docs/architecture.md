...
The `-n`/range commands add no new engine `Cmd`s: `/node_setRange`/`/node_fill` expand to `SetControl` per index, `/node_mapRange`/`/node_mapAudioRange` to `MapControl` per index (buses advancing with the control), `/bus_setRange`/`/bus_fill` to atomic writes (or `SetControlBus` in a bundle) per bus — all reusing the group-propagation and re-sort machinery of their scalar forms. The **query** commands are pure network-thread reads of the mirrors: `/synth_get`/`/synth_getRange` read the node mirror's control values and reply `/node_set`; `/bus_getRange` reads the control-bus atomics and replies `/bus_setRange`; `/node_trace` logs a node's mirror state. `/synth_forgetId` and `/buffer_close` are compatibility acknowledgements (Clausters does not reuse node IDs, and has no open streaming soundfiles yet).
