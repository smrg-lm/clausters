...
| `src/host/layout.rs` | The one placement pass: a tree into rectangles, in **physical** pixels. Each placement keeps the index of its container, so an ancestry is walked back rather than searched for (`interact`'s chain). On a `row`/`col` main axis it resolves explicit size → explicit `weight` → the widget's natural size → a share of the leftover; the cross axis fills. No measurement, no constraint solver. It also owns the units seam: the wire's own lengths are **logical** and reach physical pixels through the window's `ui_scale`, except inside a `scroll` workspace, whose content plane is physical (it has a zoom of its own and its pan is written in pointer pixels) |
...
| `track`, `clip` | `host/track.rs` — the lane, its sizeable **header** (`track::Header`: name, mute/solo, level fader) and the clip's own local time axis; a clip's roll body reuses `host/pianoroll.rs`, its curve body `host/bpf.rs`; group navigation and the axis' shared gutter in `host/timeline.rs` | a clip take: the waveform's sources; `notes`/`points` inline; edits emit `"clip"`, and a header control `"mute"`/`"solo"`/`"level"` |
