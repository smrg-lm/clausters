...
| `signal` (`view: "spectrogram"`, a `bus` + `retention`) | the **waterfall**: the same renderer over a *rolling* transform. `host/live.rs` keeps a sample history per watched **bus** (appended by stream position, so the span is true at any frame rate) and `host/waterfall.rs` keeps the transform per **widget**, analyzing each whole hop once through the very function the stored path uses (`spectrogram::column_into`) — so a retained waterfall and an offline spectrogram of the same audio are the same picture | an audio bus, retained `retention` seconds |
