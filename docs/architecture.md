...
| `src/host/instance.rs` | **What the piece is playing through.** A standalone host plays a piece the way every other endpoint does: `nodes::plan` says what the piece needs, `clausters_editing::instance::Instance` answers the difference as `Op`s, and `clausters_editing::apply::Applier` turns them into **steps** — messages, and the replies some of them wait for (a buffer's `/done` before its fill, a `/server_sync` barrier). This module is the socket: it queues the steps, sends up to the next wait and releases the rest when `Host::on_server_reply` brings the answer, from either front. The ids are the host's spaces (`ids.rs`), and every node is made at the tail of the group the transport governs, so the piece and the take monitor answer to one transport — the applier binds no second one. It also realizes a source an edit mints (a join) before the picture is redrawn: session table, take table, `/buffer_stitch`. Beside `play.rs` and not under `document/`, because what it holds is the server's and nodes are not the composition |
