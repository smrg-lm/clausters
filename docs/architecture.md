...
| `src/host/ids.rs` | **The node ids, buses and buffers the host allocates** on the server it plays through, by the one client policy (`clausters_core::ids::IdSpaces`): sized by `ServerShape::DEFAULT` until the `/server_query.reply` reshapes it, and sliced by the share a launcher gives (`--id-share`, the page bridge's `id_share`) when a script allocates on the same server. `Host::on_server_reply` is the door both fronts call for every reply: a `/node_end` gives a node id back, the server's shape reshapes the spaces, and the multitrack's waiting step is offered the rest. `Host::on_link_attached` sends the take monitor's def, registers for node ends and asks the shape — on every link, whoever launched the server, so a server that persisted an older copy of the def never answers with it; `Host::govern_transport` allocates and binds the governed group |
| `src/host/play.rs` | The **take monitor**: the buffer-player def every host sends the server it plays through, and the nodes it plays on — **one reader per channel**, which is the server's own convention rather than a shape chosen here (the buffer readers are mono; `PlayBuf`'s `chan` picks the channel), each out on the bus of the same number and freed together — except a mono take, which is heard on both sides: two readers of its one channel, to the left bus and the right. A buffer is data and does not sound — an instrument reads it — so a host that draws samples needs a def of its own to hear one, and it is the smallest that could be: `BufRd` at the transport's position, gated to the nearer of the span and the buffer's end, so a take played past its end is silent rather than holding its last sample. The space bar plays from the position cursor to the end of the take or the selection — the transport's end mark, which the engine stops on and locates back from — or, with `L`'s loop on, repeats the selection or the take; a second press stops it back at the position cursor. A pass the engine ended is told from a stop this host sent by counting the stops it sent (`play::Follow`). The view's play cursor is the transport's position, anchored from the moment the window opens. A multitrack's own instruments are the client's; this is the editor's monitor |
