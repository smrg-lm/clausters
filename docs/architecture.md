...
A UGen can ask for its node to be freed (or paused) once it finishes: the `UGen` trait has a `done(&self) -> DoneAction` hook, checked right after `process`. `EnvGen` is the first user — it returns its `doneAction` (`FreeSelf`, `PauseSelf`, `FreeGroup`, or `None`) when its last segment ends. The synth aggregates its UGens' actions (`UGenSynth::done_action` takes the strongest). The tree walk handles each action by kind: **`PauseSelf`** is applied inline — the node's `paused` flag is set and the synth is skipped (silent, but kept in the tree with its state) from the next block on (there is no `/n_run` to resume it yet, so it is terminal). **`FreeSelf`/`FreeGroup`** are recorded — id plus action code — into a **lock-free finished-node queue** (`NodeTree`'s `done_nodes`/`done_actions`/`done_count`, index reservation by `fetch_add` so the parallel workers can push concurrently). The queue is drained **once per block, after the whole tree has run** (`Engine::process_block`): `FreeSelf` frees the node, `FreeGroup` frees its enclosing group (via `free_enclosing_group`, falling back to freeing just the synth when that group is the un-freeable root), both through the ordinary `NodeTree::free` path — so the boxed synth leaves on the garbage FIFO exactly like an `/n_free`, never dropped on the audio thread. Freeing after the full walk (not mid-traversal) keeps the tree stable while it is being read, and makes a re-queue from a mid-block schedule split — or two synths in one group both requesting `FreeGroup` — harmless (freeing an already-gone id is a no-op). The queue's atomics only need to be race-free for the concurrent reservation; visibility to the drain is provided by the worker pool's join.
