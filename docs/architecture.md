...
| `src/host/elements/` | The leaves that have **moved behind the trait**: one type per widget in its own file with its tests beside it, and one row in `elements::builtin` — the table the schema consults *before* the shared registry, so a built-in name can never be shadowed by a registration. A leaf here is not an arm anywhere: `set` is its apply, `natural` its size, `draw` its frame pass, `needs` its whole declaration to the tree collectors, `hit_area` the shape it answers the pointer on (the placement rectangle by default; a disc or an ellipse for one drawn smaller or rounder than its cell, filtered once by the gesture machine rather than guarded per leaf). What is **not** on that trait is as deliberate: a picture of samples owes nineteen answers nothing else in the catalog has (what shape they are, which buffer they came from, what a span is worth, how a write lands, what the view wants next) and an element with a **measured axis of its own** four more, so those are **facets** — `Samples` and `Measured`, reached through `Element::samples`/`Element::measured` and through `WidgetKind::as_samples` at the call sites — which answer `None` by default. A capability is then *stated* rather than declined twenty-three times, and the family the audio editor grows adds to a facet instead of to every leaf's interface |
