...
| Notation layer in the shared core (`clausters_core::notation` — the score model, the SVG walk, the MEI encoder and reader, the interpreter that reads a page back into sound — with `clausters-notation` the libverovio binding; every client a shell) | done |
