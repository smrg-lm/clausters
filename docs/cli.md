...
| `--clock` | `device` \| `transport` | `device` (`transport` under `--session`) | Which counter a playhead is drawn from where no window or view names one (`/gui_headClock` names it per window or per view). `device` is the engine's sample clock, which never stops — what a host watching a live server wants. `transport` is the transport's **position**: it holds while stopped, jumps on `/transport_locate` and wraps at a `/transport_loop`'s end, all in the engine, so an editor draws that time and computes none of it. |
