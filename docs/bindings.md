...
| `clausters_editing_applier_new` | `JsApplier.new` | `idiom` — C hands back an opaque pointer, wasm a class; the applier holds the table from each operation's handle to what it became |
| `clausters_editing_applier_free` | `JsApplier` `Drop` | `idiom` — wasm frees by `Drop`, C needs an explicit `_free` |
| `clausters_editing_applier_apply` | `JsApplier.apply` | `idiom` — the steps that carry out the instance's operations, allocating from the client's id spaces; C sizes and fills a buffer (changing the table and the spaces only on the fill) where wasm returns the string |
| `clausters_editing_applier_bus` | `JsApplier.bus` | `idiom` — the control-bus run a handle became; C writes two integers where wasm returns a pair |
