...
| `clausters_multitrack_picture` | `multitrack_picture` | the rows, boxes and **curves** a piece draws as — the curves in two lists, since a track's automation is a row of its own and a region's is a layer inside its box, and a caller draws them in two different places. What a row and a box *are* is the format's business, so the standalone host and every client draw the same picture of the same piece rather than each deriving one — and a client that had to derive one would be writing the mapping a second time in its own language, which is the divergence this row exists to prevent. In **beats and seconds**: the shape is the format's and the time is the tempo map's, so a caller crosses to its own axis with the calls it already binds, and a length is the difference of two positions there |
| `clausters_multitrack_read` | `multitrack_read` | what a report of those boxes **means**, in the piece's own vocabulary. The report is the piece rather than the gesture — a move, a block drag, a trim, a split, a delete and a paste all arrive as one list — so telling them apart is one rule and not one per client: a box that changed length is a trim, one that moved or changed row is a placement, and a row that gained or lost one is a lane stated whole. A box whose name is no region id is a **new** region (a split names its halves after the box they came from), and one over samples the caller could not resolve is not invented, since the document would name a source nobody can open |
| `clausters_multitrack_read_points` | `multitrack_read_points` | what a report of the piece's **curves** means, in the same vocabulary. The twin of the row above for the light views: a multitrack reports every curve there is — the rows a track's automations draw as and the layers inside the boxes — so what this answers is the difference, one `SetAutomation` per curve whose break-points actually moved and nothing at all for a hand that looked without editing. A name that is no automation's id is dropped rather than minted, because a curve is declared by whoever holds the piece and a hand that dragged a break-point made no new one |
| `clausters_multitrack_read_rows` | `multitrack_read_rows` | what a report of the piece's **rows** means, in the same vocabulary — the third reader, and the one that makes the stack of tracks editable rather than only readable. The report is every row, in the order they are shown, so what comes out is the difference and it is **one** `SetTracks` whatever changed: a name that is a track's id is that track (with the strip's mute, solo and level written onto it), a name that is no track's id is a track a hand made (minted here with one empty lane, since a track that could hold nothing is not one), a track the report leaves out is **gone with its boxes**, and the order is the report's. The label is drawn and never written back: a row's label is the track's name where it has one and a made-up `track N` where it has not, so believing the report would put that string into the document the first time anything else moved. The ids a new track needs are minted from the piece itself, so there is nothing for a caller to reserve |
