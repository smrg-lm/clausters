...
| `clausters_apps_samples_editor_call` | `JsSamplesEditor.call` | `idiom` — **one verb of the editor**, as JSON: `sync` the facts a client holds, read or replace the measure stack (`layers`), compose the `window`, correct a widget (`props`), read and answer an `event`, `acknowledge` a step of the history. An event's blobs cross decoded to their numbers, which is the wire's framing and each client's. The write a stroke made and the history are not in the editor: the samples are in a server buffer, so a turn answers the write to carry out and the entry to record. One door for the multitrack editor's reason. C sizes and fills a buffer where wasm returns the string |
