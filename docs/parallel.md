...
Without `--workers` (the default) everything stays sequential; `/group_parallel` is then a no-op flag, accepted and remembered. The flag is schedulable in timed bundles and valid in NRT scores, like `/group_sortMode`.
