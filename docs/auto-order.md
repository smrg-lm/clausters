...
- Both clients spell it as a verb on the group — `group.auto_order()` in Python, `group.autoOrder()` in TypeScript, either with `False`/`false` to hand the order back. `Group.parallel` is its pair: the same analysis, read for *what can run at once* rather than *in what order* (see [`parallel.md`](parallel.md)).
