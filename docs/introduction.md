...
- **Two def formats, both loaded hot over OSC**: a flat **SynthDef JSON** list of UGens (`/def_send synth`, default `synth` feature), and **Faust** defs — Faust source or a JSON box tree — JIT-compiled with the LLVM backend (`/def_send faust`, `faust` feature). The two families are independent build features: enable both or ship a single-family server.
- **Sample-accurate scheduling**: NTP-timetagged bundles split the audio block at the event's exact frame, plus a direct **sample clock** (`/clock_query`, `/sched_at`) as a drift-free client timebase.
- **Offline (NRT) rendering**: the same engine renders scores to WAV without an audio device, bit-identically to a live take.
- **Auto-sorted groups** (`/group_sortMode`): execution order inferred from the buses each def reads and writes — no manual `/node_before` bookkeeping.
- **Parallel groups** (`/group_parallel` + `--workers N`): independent children run on several cores, **bit-identical** to the sequential result.
- **Control/bus mapping** (`/node_map`, `/node_mapAudio`): any control or Faust parameter can track a control or audio bus, live, every block.
