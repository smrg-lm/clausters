...
buffer with ``/b_allocRead`` and loops it from a Faust def that reads it through
`soundfile`, all driven by the Python client's own resources (buffer allocator,
def sending with the ``/done`` barrier, live ``/n_set`` automation).
...
Two things worth knowing:
  ...
  * **The bind is a snapshot taken at ``/s_new``.** When a synth instantiates,
    ...
    touch a voice already playing -- re-``/s_new`` to pick up the new samples.
...
def _alloc_read(server: Server, bufnum: int, path: str):
    ...
    if addr == "/fail":
        raise CommandError(f"/b_allocRead {bufnum} failed: {args}")
