...
- ⬜ **S12 — An edit is an NRT job over a file, and a buffer stays replaceable** *(opened 2026-08-15 as "a buffer write costs the samples written"; **rewritten the same day**, because the premise it rested on was rejected rather than refined. What was measured is kept below: the measurement is what makes this decision safe instead of merely tidy, and the first shape is kept named because a milestone that quietly changed its mind teaches nothing.)*

  **The decision, and it is the one that keeps the real-time path solid.** A pool buffer is **immutable and replaceable**, exactly as `src/dsp/buffer.rs` has always said, and nothing the sample editor needs changes that: no writable buffer, no atomic storage, no copy-on-write block table, and **no second buffer kind in the pool**. `Osc`, `VOsc`, `Shaper` and `Conv` therefore need no answer at all — they keep `Buffer::data()` and today's speed — and the question this milestone was opened to decide **dissolves rather than being resolved**, which is the better outcome of the two and the reason to write down how it happened. A second kind would have been the expensive answer, not the cheap one: it puts the editing concern inside the RT read path, where every reader has to know which kind it holds, to serve a use case that never needed the audio thread's help.

  **What was measured, and what it actually established** (2026-08-15, release build; writes of 1 ms fired back to back and closed by one `/server_sync`, so the NRT queue's own serial rate is what shows):
  ...
  The cost is linear in the **buffer** (~0.29 ms per MB) and flat in the span, and three quarters of it is allocating and faulting in a second take rather than the memcpy (a fresh 110 MB copy measures 31.2 ms against 8.05 ms into a destination that already exists). Mutable storage was also measured and was affordable — an in-place write costs 0.01 µs and charges an interpolated read +3%, a block table 6.3 µs and +20% — so **this decision is not the cheap one winning; it is the cheap one being refused on other grounds.** What the numbers establish is narrower and more useful: 33.8 ms is a defect **only under the assumption that every stroke writes the take**, and that assumption was never the architecture's. The working copy leads while the session is open (`crates/clausters-document/PLAN.md`, O8), a stroke is heard against the span it just drew, and a take's buffer is therefore replaced **once per confirmed edit** — 33.8 ms, once, against a gesture the user has just finished — which is not a cost worth a new storage model.

  **Where each half of an edit lands, which is the whole design:**

  | what | where it happens | what it costs |
  |---|---|---|
  | the stroke while the hand is moving | the host's pending overlay | nothing crosses |
  | hearing that stroke | a scratch buffer the length of the span | 2.26 ms, the existing replaceable model |
  | the edit itself | an NRT job over the working copy | disk-bound, off both other threads |
  | confirming it | the file is written; a take held in the pool is replaced whole | 33.8 ms, once |
  | playing a long file | `DiskIn`, no pool buffer at all | see the entry under "Future directions" |

  **What the server grows, then.** *One place performs audio processing and it is the server* is already settled (`crates/clausters-document/PLAN.md`), so the editing verbs are the server's rather than a client's: a span of a file gained, faded, reversed, silenced, or replaced by drawn samples. They are **NRT jobs**, which is the thread that already opens files, already serializes work in submission order and already answers `/done`/`/fail` — so the whole cost of the feature lands where that machinery already exists. **The RT path gains nothing at all**, which is the point: the audio thread never sees an edit, and that is precisely what keeping the buffer replaceable buys.

  **The one line not to cross, named because it is where this would go wrong.** The server today knows nothing about *sessions* — what the working copy is, what has been confirmed, what to clean up — and the document crate owns every bit of that. So the editing vocabulary has to be **file to file with no session state**: an operation names its input, its span, its parameters and its output, and the server forgets it happened. The moment the server starts holding working copies it grows a second session model beside the crate's, in the process that is hardest to reason about, and that is the complication this decision exists to avoid.

  **What is still open**, and it is design rather than measurement: the vocabulary itself (which verbs ship first, and whether "replace this span with these samples" is one of them or is `/buffer_setRange`'s file-side sibling); whether an operation writes in place into the working copy or produces a new file the caller swaps; and how a peaks cache is kept current beside it, since the host redraws from that and not from the file (`peaks::update_range` is D1's half of the same sentence). **Acceptance:** a drawn edit on a five-minute take is applied, heard and confirmed without any pool buffer being written per stroke; the RT read path is untouched, byte for byte, and `tests/rt_safety.rs` and the golden renders say so; the server holds no state about the edit once it has answered.

  **What this decision costs, stated rather than hidden: `RecordBuf` can never exist.** An immutable pool buffer cannot be written from the audio thread, so recording into a buffer and playing it in the same breath — the looper idiom, and what scsynth's `RecordBuf` is for — has no home here. Recording to *disk* is covered (`DiskOut`), and a buffer filled from the NRT side is covered, but the live looper is not. That is a real capability given up for the invariant, it is the only one, and it is recorded here so that the day someone wants a looper the answer is a deliberate reopening rather than a surprise.
