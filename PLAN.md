...
- ⬜ **A transport addressed in samples still needs a beat grid** *(noticed 2026-08-16 while closing T5, and left open by it on purpose)*. `/transport_play`, `/transport_stop`, `/transport_locateSample` and `/transport_loop` all refuse until `/transport_set` has defined a grid, because every `/transport_*` command but that one always has. For an audio editor that means declaring a tempo it never reads — `examples/transport_seek.py` sets one and says in a comment that it is arbitrary, which is the smell. The rolling state and the piece's position have nothing to do with beats; only `/transport_locate` (the beat spelling) does. Fixing it means the transport existing before a grid does — `defined` becoming a flag on a transport that is always there, rather than the `Option` standing in for both — which is a change to commands that already work and had no business riding in on T5.
