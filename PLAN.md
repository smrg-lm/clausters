...
- ⬜ **`--shm` only ever creates a segment, so two servers cannot share one** *(found 2026-08-17, answering the user's question about how the processes are arranged)*. `clausters --shm <path>` calls `Segment::create_full`, which **truncates** the file and re-initializes the header. That was right while a segment was a transport — one server, its own ring — and it is wrong now that the segment indexes the **material** (S19): the process that may be killed and restarted is the one that would wipe what everybody else is editing.
  ...
  What is missing is the other half of the door: `--shm` that **opens** an existing segment, validating magic and ABI exactly as a peer's `Segment::open` already does, and creating one only when there is none. With it the arrangement H6 wants becomes expressible — the editor (or the on-demand session it opens) creates the segment and owns the material, and every server *attaches* — and its acceptance stops being a diagram: killing the RT server and starting another against the same segment leaves the material where it was.
