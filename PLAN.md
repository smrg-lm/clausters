...
- ✅ **S8 — FFT/IFFT and the spectral (`fr`) chain, prepared on the S6 `/cmd`/`/u_cmd` surface** *(**DONE 2026-07-03** — `FFT`/`IFFT` + `PV_MagAbove`/`PV_MagBelow`/`PV_BrickWall` in `src/dsp/spectral.rs`. The transform and the smoothing windows are single-sourced in `clausters-core` — `fft::rfft_into`/`irfft_into` (the user confirmed `microfft` 0.6 already ships an `inverse::ifft_*`, `no_std` + zero-alloc, normalized by `1/N`; no new dep, the "forward-only" note was stale) plus a new `window` module (Hann/Sine/Welch/Hamming/Blackman) shared with the clients for binary parity. **Deviation from scsynth, decided with the user:** the spectral frame does **not** live in a client-allocated pool buffer (which is immutable) — it is **synth-private scratch** (`SpectralChain`: packed `[dc,nyq,re,im,…]` frame + `ready` + hop `advance`), allocated at synth instantiation and freed with it, the `LocalIn`/`LocalOut` `locals` pattern = SuperCollider's `LocalBuf`; no `/b_alloc`, the sample pool stays immutable. `FFT` slides an input ring + analysis window, emits one frame per hop (quantized up to the slice, as scsynth); `IFFT` overlap-adds **window-normalized** (÷ window energy → unity-gain reconstruction, one window of latency) with an output FIFO, kept in lockstep with `FFT` by the chain's `advance`. New `ExecMode::Spectral` + `SpectralRole` descriptor field + `UGen::process_spectral`; the compiler assigns each `FFT` a chain slot (`SynthDef::spectral_sizes`) and the downstream `PV_*`/`IFFT` inherit it + the window size/type (size given only on the `FFT`); `UGenConfig`/`UGenSpec` gain `fft_size`/`hop`/`wintype`. `/u_cmd … window <wintype>` swaps the window live — the **first real consumer** validating S6's typed per-UGen command surface. Tests: `tests/spectral.rs` (tone round-trip within tolerance, `PV_BrickWall`/`PV_MagAbove` band attenuation, compiler validation, `/u_cmd` swap), `tests/rt_safety.rs` no-alloc chain scene, core `fft`/`window` unit tests. Docs in `schemas.md`/`architecture.md` (the `fr` chain contract + the synth-private-scratch deviation), a `GUIA.md` S8 section + row, the `fft` demo in `examples/json_client.py` (E2E-verified). See LOG.md)*: scsynth's frequency-domain processing bookends a chain of `PV_*` UGens between **`FFT`** (window an audio input, transform, pack the complex frame into a buffer) and **`IFFT`** (inverse-transform + overlap-add back to audio). The chain is **not block-rate**: `FFT` emits one spectral frame per **hop** (the frame-rate `fr` idea flagged in S1 — the "not evaluated every block" substrate the `dr` driver already established, here a hop gate rather than a demand pull), and every `PV_*` UGen in between runs only on the blocks a fresh frame is ready. Build on what S1/S5/S6 put in place: (a) `FFT buf, in, hop, wintype, active, winsize` / `IFFT chain, wintype, winsize` — the transform is **pre-planned at synth init** (`ir` rate / network thread, where allocation is legal — an FFT planner allocates), the per-hop transform reuses pre-allocated scratch so the audio thread stays allocation-free; (b) the **spectral-frame buffer format** (the complex bins packed into an `/b_alloc`'d buffer, reusing S5's buffer/wavetable NRT machinery where it overlaps, cartesian/polar as scsynth); (c) the **`/u_cmd` typed per-UGen command surface from S6 as the configuration/query channel** for these UGens (window type, chain binding) — S8 is the first real consumer that validates that mechanism's design (S6 explicitly landed the mechanism "for future FFT/streaming UGens"); (d) a minimal `PV_*` set proving the chain (e.g. `PV_MagAbove`/`PV_BrickWall`/`PV_MagMul`). Deliverables: `FFT`/`IFFT` + the frame buffer format + a couple `PV_*` UGens; tests (an `FFT`→`IFFT` round-trip reconstructs a tone within tolerance; a `PV_` filter attenuates a band; a no-alloc scene proving the per-hop transform allocates only at init); `docs/schemas.md` (the FFT chain wire format + the `PV_*` list) and `docs/architecture.md` (the `fr` chain contract, extending S1's `dr` sub-list note); an example.

- ✅ **S9 — Side-effect UGens (no `Out` required): the reply/observation and write-only families** *(**DONE 2026-07-03** — the reply/observation sub-family shipped: `SendTrig`/`SendReply`/`Poll` (`src/dsp/reply.rs`), each detecting a `≤0`→`>0` trigger and buffering an inline `Copy` `ReplyMsg` (fixed name + value array, `REPLY_BUFFER_LEN` 8/block) that leaves the audio thread the same RT-safe way as `/n_go`/`/n_end`: the tree marks reply-producing synths into a lock-free `reply_slots` queue (`fetch_add` reservation, worker-safe) and `Engine::process_block` drains it once after the block into a new SPSC reply FIFO (2048, best-effort); `collect_garbage` turns each into OSC — `/tr nodeID id value` (SendTrig), a custom-address `cmdName nodeID replyID value…` (SendReply), a `tracing` console line + optional `/tr` (Poll). `UGen` gained `is_reply`/`drain_replies`, `SynthNode` `has_replies`/`drain_replies` (UGenSynth precomputes `has_reply_ugens`), and a `label` field threads `UGenSpec`→`UGenConfig`→`compile` like `op`. C19 done: the Python `SynthDef` now takes graph **roots** (no `Out` required) with `send_trig`/`send_reply`/`poll` builders. Tests: `tests/osc.rs` +3 (`/tr`, custom address round-trip, Poll trigid), `tests/rt_safety.rs` +1 (no-alloc under an Impulse firing all three), `test_synthdef.py` +1; docs in `schemas.md`/`architecture.md`, a `GUIA.md` S9 section + row, the `replies` demo in `examples/json_client.py`. Write-only UGens (`Out.kr`/`LocalOut`/`OffsetOut`, `DiskOut`/`RecordBuf`) deferred with the buffer/streaming work as planned. See LOG.md)*: a whole family of UGens exists whose purpose is a **side effect**, not audio on an output bus — they send OSC replies, post to the console, or write to a buffer/control bus, and a valid SynthDef may contain *only* such UGens with no `Out` at all. Two verified sub-families. **Reply/observation UGens** — `SendReply trig, cmdName='/reply', values, replyID` (send an arbitrary-arity OSC message when triggered), `SendTrig in, id, value` (send `/tr nodeID id value`), `Poll trig, in, label, trigid` (periodically post a UGen's value to the console, optionally replying): they run on the audio thread but their effect leaves through the **existing RT-safe reply/garbage FIFO to the network thread** (the same path `/tr`/`/n_end` already use — never I/O or allocation on the audio thread), and they lean on the S6 `/cmd`/reply plumbing for the typed message surface. **Write-only UGens** — `Out.kr`/`LocalOut`/`OffsetOut` writing buses, `DiskOut`/`RecordBuf` writing buffers/files — output-less in the `Out.ar`-to-hardware sense. The **server already permits output-less defs** (`compile` requires ≥1 UGen, never an `Out`), so the client side is purely a **relaxation**: the Python def/tree builder must not assume a SynthDef has an `Out` (full write-up in `clients/PLAN.md` **C19**). Deliverables: the server grows the reply/observation UGens on the RT-safe reply FIFO (`SendReply`/`SendTrig`/`Poll` first, the write-only ones as the buffer/streaming work lands) with tests (a `SendTrig` produces the expected `/tr`; a `SendReply` round-trips its values; a no-alloc scene proving the reply path never allocates on the audio thread) and `schemas.md`/`architecture.md` coverage of the reply-from-the-audio-thread contract; the client drops the `Out` assumption (C19). This item arrived with the S8 request, hence its place at the tail of the S track.
