...
``"note"`` events) is the Python example ``clients/python/examples/panels/piano.py``.
...
- **Browser, as a web component** (the wasm engine in an AudioWorklet):
  ...
  and open ``http://localhost:8000/examples/panels/piano/`` — `index.html` here
...
def main():
    ...
    print("    http://localhost:8000/examples/panels/piano/\n")
