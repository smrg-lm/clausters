 ...
 * - a **step** of the pile: the direction, whether anything moved, and the
 *   labels on either side of the cursor;
 ...
 *
 * **Silent unless asked**, like every other area of `base/log.ts`, of which this
 * is one: a page sees nothing, and a caller routes it wherever it routes the
 * rest of its logging. `CLAUSTERS_LOG=gui.editing` arms this one alone,
 * `CLAUSTERS_LOG=1` arms everything, and {@link watch} is the same door from a
 * page. In a browser the environment is `globalThis.CLAUSTERS_LOG` or a
 * `?clausters-log=` in the URL.
 *
 * @module
 */

import { Area, area, watch as watchArea } from "../../base/log.ts";
...
export function watch(sink?: LogSink): Area {
    return sink === undefined ? watchArea("gui.editing") : watchArea("gui.editing", sink);
}
