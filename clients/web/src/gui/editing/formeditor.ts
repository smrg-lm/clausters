...
export class FormEditor extends Editor<Element> implements Adopting {
     ...
     * them — so nothing here reads the lane the message was addressed to: each
     * clip is found by its own id, and the placements go into one transaction
     * whatever aggregate they belong to.
     *
    ...
    private setPlacements(aggregate: Aggregate, members: Record<string, unknown>[]): boolean {
        ...
        for (const m of members) {
            ...
            if (target !== undefined) {
                ...
                // **And the index has to learn the handle that holds it now.** A
                // restored placement is a *new* handle for an old node, and the
                // index maps a node to the object an intent writes to — so left
                // alone it goes on naming the handle this restoration replaced,
                // and every later `place` on that clip is applied by the crate,
                // answered `true`, and written onto a placement nobody is
                // drawing. That is a clip that cannot be moved and says nothing
                // about why.
                this.editing.index(target[2], aggregate, restored);
            }
        }
        return true;
    }
