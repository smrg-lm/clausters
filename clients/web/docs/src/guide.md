...
Everything that is a *value* or a *time* comes from `clausters-core` compiled to wasm, the same crate the server itself runs and the same one the Python client reaches through its C ABI. The OSC bytes, the beat/second/sample conversions, the bar grid, the NTP timetag, the beat-ordered scheduler queue, the seeded random stream, the builtins — the operators and conversions (`midicps`, `dbamp`, …), the **range maps** (`linlin`, `linexp`, `explin`, `expexp`, `lincurve`, `curvelin`, `range`, `exprange`) and the **perceptual frequency scales** (`cpsmel`/`melcps`, `cpsbark`/`barkcps`) — the node/bus/buffer allocators: all of them are calls into that bundle.
...
A `FaustDef` is built three ways, all equal citizens and all of them compiling in a tab (`libfaust-wasm` runs in the NRT worker, and the module it emits is linked into the engine's own memory): a **signal expression** (`fromSignals`, built with `defs.signals`), a **box tree** (`fromBox`, built with `defs.boxes`) or **Faust source** (`fromSource`). `GraphDef` wires several defs of either family into one named, instantiable configuration with a port surface. A member of one may itself be a `GraphDef` (`kind: "graph"`) — so an effect chain and a nested graph are one mechanism, not two — and a member marked with a `slot` is one there is a **changing number of**, built on demand with `group.addSlot(name)`: a voice of an instrument, a clip on a track, an effect in a chain. A slot can move to another instance of the same def with `slot.moveSlot(instance)` — a clip dragged to another track — re-wired there rather than built again. The nesting is of authoring: a port resolves once, at instantiation, to the controls of actual nodes however many levels down they were written.
...
Above that sit `Event` and `rest`, the value patterns (`Pseq`, `Pser`, `Prand`, `Pwhite`, `Pseries`, `Pgeom`, `Pfunc`, `Pn`, `Pconst`), `Pbind`, and the seekable counterpart: `Timeline`, a beat-sorted plan with its own tempo map that `Timeline.fromPattern` can bounce a pattern into, and that plays itself with play/pause/stop/locate/loop. Random values come from the stream a routine derives when it is created, so `seed(n)` replays a whole piece. The model itself — what an event's keys mean, how `dur` and `sustain` differ, what `Pbind` does with a pattern of patterns — is the Python book's [routines and clocks](https://clausters-python.readthedocs.io/) chapters; it is the same model.
