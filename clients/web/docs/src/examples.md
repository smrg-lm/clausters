...
| [`examples/editor.html`](https://github.com/smrg-lm/clausters/blob/main/clients/web/examples/editor.html) | A sound file decoded by the browser into a server buffer, drawn by the host as a linked `waveform` and `spectrogram`, with a transport over it: play anchors the playhead to the engine clock, pause freezes the node with its position, and a dragged selection plays as a loop. |
