...
handles, or as a plain script -- ``python clients/python/examples/editors/bpf.py``.
...
win = view(
    ...
    title="BPF envelope -> EnvGen", w=640, h=460, layout="col").open()
...
def play(*_):
    ...
    SynthDef("bpf_env", out(0.0, sig), out(1.0, sig)).send(server)
    Synth("bpf_env", server=server)
