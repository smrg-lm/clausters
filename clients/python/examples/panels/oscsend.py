...
``python clients/python/examples/panels/oscsend.py``. `Session.gui` boots the GUI
...
def beep() -> SynthDef:
    ...
    return SynthDef("oscsend_beep", out(0.0, sig), out(1.0, sig))


beep().send(server)          # blocks until /done
synth = Synth("oscsend_beep", {"freq": 220.0}, server=server)
...
win = view(
    ...
    title="OSC message field", w=560, h=360, layout="col").open()
