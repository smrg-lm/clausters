#!/usr/bin/env python3
"""Freeze a generative run and hear that it *continued* rather than restarted.
...
the audio is produced as it sounds, so the run's position **is** the state
of the running nodes and no number moves it. The only thing a transport can
honestly do to such a run is stop it and let it carry on.
...
into a transport that is not moving. `TempoClock.freeze` holds the beat instead, and
...
two takes are one run interrupted, never two different runs.
...
# that group (the event's ``target``), so freezing the group freezes the sound.
...
print(f"governing group {governed.id}")
...
def texture(n: int):
    ...
    for i in range(n):
        Event(instrument="cloud", target=governed.id,
...
# stops scheduling into a transport that is not moving. The look-ahead already in
...
if __name__ == "__main__" and not hasattr(sys, "ps1"):
    ...
    print("sound up - server.transport_stop() / transport_play(), teardown() to end")
