...
This is the level-2 half of the patcher; `examples/gui_patch1.py` is level 1
...
Run it as a script (``python gui_patch2.py``) or cell by cell (``# %%``). It
...
# A little voice, built to show every cord type in one picture: audio cords
...
def voice() -> SynthDef:
    ...
    return SynthDef("voice", out(0.0, sig), out(1.0, sig))
...
patch = DefPatch.from_synthdef(voice_def)
print("voice decoded into", len(patch.boxes), "boxes,", len(patch.cords), "cords")
...
rebuilt = patch.to_synthdef("voice")
print("round trip reproduces the spec:", rebuilt.spec() == voice_def.spec())
...
# %%
def faust_voice() -> FaustDef:
    """A Faust FM-ish tone: a modulated sine, gained by a slider."""
    ...
    return FaustDef.from_signals("faust_voice", sin(freq + modulator) * gain)


faust_def = faust_voice()
faust_patch = DefPatch.from_faustdef(faust_def)
print(f"\nfaust_voice decoded into {len(faust_patch.boxes)} boxes, "
...
# %%
def show(host, a_def, note: str) -> None:
    """Open a def's Def-view and block until its window is closed. The panel is
    captioned by the def *kind* (``synthdef`` / ``faustdef``), not the def name —
    the window title carries the name."""
    print(f"\n{note} — close the window to continue")
    win = a_def.plot_def(host=host)
    while True:
        msg = host.poll(0.1)
...
if __name__ == "__main__":
    try:
        with GuiHost.boot() as gui:      # no server: the Def-view needs none
            show(gui, voice_def, "the SynthDef 'voice' (level 2)")
            show(gui, faust_def, "the FaustDef 'faust_voice' (level 2)")
