...
the three stages below are its three groups of cells: bounce, read back, load
and play.
"""
...
# %% [markdown]
# ## 1. Bounce it
# The server writes the file; the stats come back here.
...
# %% [markdown]
# ## 2. Read it back, through the server's decoder

# %%
audio = read_soundfile(wav)
left, right = channels(audio.samples, audio.channels)
print(f"read back: {audio.frames} frames x {audio.channels} ch, "
...
# %%
def run():
    """Play the take back from the buffer."""
    ...
    node.free()
    print("done - the bounce, the file and the playback were one artifact.")
...
if __name__ == "__main__" and not hasattr(sys, "ps1"):
    ...
    print("buffer loaded - run() to play it back, live.close() to end")
