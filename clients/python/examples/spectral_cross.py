...
Runs from the *installed* package, offline, like ``spectral.py``::
    ...
    python clients/python/examples/spectral_cross.py out.wav

Where ``spectral.py`` filters one chain, this def runs **two** FFT chains and
...
def main():
    ...
    def sequence():
        ...
        for midi, dur in [(57, 2.0), (60, 2.0), (64, 2.0), (62, 2.0)]:
            ...
            session.server.send_bundle(("/n_set", voice.id, "freq", freq))
            yield dur
        # Freeze the last envelope: the melody stops, the texture holds.
        session.server.send_bundle(("/n_set", voice.id, "freeze", 1.0))
        yield 4.0
