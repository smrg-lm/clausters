...
# shared case, which is the whole reason these exist beside the private family.
# No ``max_delay``: the buffer's own frame count is the bound. The buffer is
# **not** zeroed by the UGen (allocating and clearing it are yours), a missing
# buffer or a channel it does not have plays silence, and ``bufnum``/``chan``
