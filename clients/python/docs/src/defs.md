...
This page is the conceptual map and the catalog of what each callable does. The exact wire format — the JSON node shapes, the UGen registry, the `/def_send faust` / `/def_send synth` signatures and their `/done` / `/fail` replies — is specified in the **[Clausters server book](https://clausters.readthedocs.io/)** (the schemas / OSC reference chapter); this client is one consumer of it. The generated [API reference](api/clausters.defs.md) carries the per-symbol signatures.
...
| Bitwise | `& \| ^ << >>` |
| Unary methods | `.abs() .floor() .ceil() .sin() .cos() .tan() .asin() .acos() .atan() .exp() .log() .log10() .sqrt() .asinteger() .asfloat()` |
...
| | `clip_count(signal, ceiling=1.0, run=3)` | **how many times the signal was flattened**, since the pass began: a run of `run` consecutive samples at or over `ceiling` is one over, however long it lasts. A lone sample at full scale is not clipping — the engine is floating point — so the run is what a red lamp is actually reporting; `run=1` is the pessimistic meter. It walks samples rather than blocks, which is what a block peak has already lost. Pair it with `out_ctl` and hand the bus to a `meter` widget's `clip` |
| | `true_peak(signal, decay=20.0, hold=0.0)` | a meter's level over the **reconstructed** signal: `meter`'s ballistics, but the block's true peak (the ITU-R BS.1770-4 filter) rather than its largest sample, so the reading is in dBTP. Pair it with `out_ctl` and a `meter` widget at `rate="control"` with `peak="true"` |
...
- **`out(bus, chans)` turns channels into buses**: one writer per channel on `bus`, `bus+1`, … (`replace_out`/`out_ctl` likewise). The base bus must be a constant.
- **A channel list is an expression**, so the verbs take one directly, with no def of its own: `play(sine(440).dup())` sounds in stereo, `plot(pan2(sig, pos))` shows two lanes, `render(chans(a, b), channels=2)` bounces the pair. The coercion lays the members that are *not* already sinks on buses 0, 1, … in order — a member that is one (an `out`, a `send_trig`) already knows where its data goes, passes through as a root, and consumes no channel, so a reporter in the list does not push the audio off bus 0.
