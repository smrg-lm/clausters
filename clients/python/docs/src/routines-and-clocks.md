...
- `clock.ramp_tempo(bps, over=beats)` is the other tempo gesture: an accelerando or a ritardando, from the tempo now to `bps` across `over` beats, holding it after. Where `set_tempo` is a step *from now on*, this is a shape written over a stretch of the piece.
