...
- **`play` only schedules; something else has to drive the clock.** A routine played onto a clock nobody drives sits in the queue: no exception, nothing logged, and the result sounds exactly like silence. `clock.run(seconds)`/`clock.start()` drive it live and `clock.render()` drains it offline — through a session, `session.run(...)`, `session.start()` and `session.render()`. A program that ends with a session's clock still queued and never driven says so on stderr, since that is the one case that can only be a mistake.
...
- `clock.set_tempo(bps, over=beats)` is the same verb writing a **shape over a stretch**: an accelerando or a ritardando, from the tempo now to `bps` across `over` beats, holding it after. `unit=SECONDS` gives the stretch in wall clock instead, solved exactly rather than searched for, and `curve=` picks the shape — `LINEAR`, `EXPONENTIAL` or a numeric curvature, `Env`'s own vocabulary. Hand it an `Env` of tempos and the whole shape is written in one call; it must be of finite duration, since a piece's tempo has no gate to sustain on. The formulas behind all of it are in `clausters.base.time`.
