# Grouping: placing materials in time

The five primitives adorn things the client already had. The **`Group`** is
the one genuinely new structure of the model: it places materials by an
offset, recursively — a group is itself a material, so a phrase sits inside a
section inside a piece, and the same operations work at every level.

## The song tree

Each lane of the piece is a group placing one material (the drums place the
take twice), and the song is a group placing the lanes:

```python
from clausters.model import Group
...
A child is seeded as a `(offset, material)` pair, a bare `Material` (placed at
offset 0), or a `(offset, dur, material)` triple — the third form also fixes a
...
the model is absolute until realization adds the offsets up.

## Placements are edits: the member handle

A group is not frozen. `add` places a material and returns a **handle** — a
...
`members` reads the placements as `(offset, dur, material)` triples; `handles`
...
A placement `dur` **trims** what the material plays — the DAW rule: pull in a
clip's edge and you hear less of it, but the material underneath is untouched.
Watch it on the model directly:

```python
from clausters.model import flatten

drums.move(member, 4.0, 1.0)             # same place, but only 1 beat of it
print(flatten(drums))                    # the second event now has dur=1.0
print(take.to_event()["dur"])            # 2.0 — the material is untouched
drums.move(member, 4.0, 2.0)             # restore the full take
```

The trim never rewrites the material: the shortened event is a copy made at
...
together are one thing on the timeline, which is how the model attaches an
envelope to the voice it shapes — [the automation page](automation.md) uses it.

(There is a second grouping **kind** besides the default `compositional`: a
...
Realization will flatten the tree — accumulate every nested offset into
...
(default 0.8), and lengths in the model are what you *hear*, a rule you will
meet again. `to_timeline(song)` builds the same thing as a `Timeline`, which
is precisely what a playhead plays — and that is the next page.

Next: [Realization: hearing the model](realization.md).
