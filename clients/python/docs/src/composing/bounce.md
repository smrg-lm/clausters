...
The last change of state: the whole piece, realized offline. The promise made
on the realization page comes due here — RT and NRT are the *same* flattening,
differing only in the destination, so the render is sample-identical to what
you heard. There is no export path; there is `realize`, pointed at an offline
session.
...
- the sweep was `prepare`d on the *live* server, and the drone's voice
  ...
  re-point the voice at the bus the offline pass allocated — then realize,
...
playhead = song.realize(offline.server, offline.clock)
```
...
Both realizations converge on the same score; the engine that renders it is
...
- [Composition: the model and the multitrack editor](../composition.md) — the
  concepts, argued rather than exercised.
- [The glossary](glossary.md) — every term this section used, pinned in one
  place.
- The [API reference](../api.md) for `clausters.model`, `clausters.gui` and
