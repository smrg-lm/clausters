...
| `waveform` | heavy GPU view | Editor-grade min/max peak waveform of a buffer/file/blob: multichannel lanes (stacked or overlaid), LOD-crossfaded zoom, adaptive rulers, selection, playhead, amplitude zoom/pan (G20-G20c); linked groups and the RMS body planned (G20d-G20e). |
| `spectrogram` | heavy GPU view | Editor-grade STFT time-frequency view, same sources and chrome as `waveform` plus a Hz ruler and frequency zoom/pan (G20-G20c); linked groups planned (G20d), a constant-Q analysis mode experimental (G20f). |
