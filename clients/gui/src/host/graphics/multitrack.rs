//! The arrangement a `multitrack` owns: lanes, clips, and where they land.
//!
...
//! while this holds the arrangement as **data** the way [`super::pianoroll`]
...
//! widgets there was nobody to report *the arrangement*, so a gesture reported
...
/// One row of the arrangement: a track's lane, named and sized.
///
...
/// The whole arrangement: the lanes in the order they are stacked, and the
/// clips on them.
///
/// **Two lists rather than clips nested inside lanes**, and that is not an
/// implementation detail: a clip changing lane is then one field of one clip
/// rather than a removal and an insertion, which is exactly the edit that had
/// no way to report itself before. It is also what makes the two edit-backs
/// independent — a fader moved resends the lanes and not every clip.
#[derive(Debug, Clone, PartialEq, Default)]
pub struct Arrangement {
    /// The lanes, top to bottom.
    pub lanes: Vec<Lane>,
    /// The clips, in no particular order: each names its own lane.
    pub clips: Vec<Clip>,
}

impl Arrangement {
    /// The lane of this name, if it is here.
    ...
    /// The clip of this name, to be edited.
    pub fn clip_mut(&mut self, name: &str) -> Option<&mut Clip> {
        self.clips.iter_mut().find(|c| c.name == name)
    }

    /// The clips on `lane`, in the order they are drawn — by position, so a
    /// written arrangement is stable and two of them overlap predictably.
    pub fn clips_on<'a>(&'a self, lane: &'a str) -> impl Iterator<Item = &'a Clip> + 'a {
        self.clips.iter().filter(move |c| c.lane == lane)
    }
    ...
    /// lengthens the piece by its whole length, which is the number a ruler and
    /// a scroll have to size themselves against.
    pub fn extent(&self) -> f64 {
        self.clips.iter().map(Clip::end).fold(0.0, f64::max)
    }

    /// How tall the stack is with `gap` between lanes — what a scroll's content
    /// height is, and what says whether it scrolls at all.
    pub fn content_height(&self, gap: f32) -> f32 {
        if self.lanes.is_empty() {
            return 0.0;
        }
        let lanes: f32 = self.lanes.iter().map(|l| l.height).sum();
        lanes + gap * (self.lanes.len() - 1) as f32
    }

    /// Where each lane lands inside `rect`, scrolled down by `scroll` pixels.
    ///
    /// One entry per lane, in stacking order, **including the ones off the top
    /// or the bottom** — a caller that draws skips what does not intersect, and
    /// a caller that hit-tests needs the same rects the drawing used or the two
    /// disagree in exactly the cases nobody tests.
    pub fn stack(&self, rect: Rect, scroll: f32, gap: f32) -> Vec<Rect> {
    ...
    ///
    /// `min_w` is what keeps a very short clip from vanishing at a wide zoom:
    /// a box narrower than that is drawn at that width, because a clip nobody
    /// can see is a clip nobody can grab.
    pub fn clip_x(&self, clip: &Clip, body: Rect, nav: &View, min_w: f32) -> Option<(f32, f32)> {
        super::track::clip_x_range(body, nav, clip.place.offset, clip.place.dur, min_w)
    }

    /// The `lanes` wire form: the flat `name label height mute solo gain`
    /// sextuple array.
    ///
    ...
    pub fn lanes_json(&self) -> Value {
        let mut out = Vec::with_capacity(self.lanes.len() * 6);
        for l in &self.lanes {
    ...
    /// The `clips` wire form: the flat `name lane offset dur start label`
    /// sextuple array.
    pub fn clips_json(&self) -> Value {
        let mut out = Vec::with_capacity(self.clips.len() * 6);
        for c in &self.clips {
...
mod tests {
    ...
    fn piece() -> Arrangement {
        Arrangement {
            lanes: vec![Lane::new("noise", 100.0), Lane::new("tone", 60.0)],
            clips: vec![
                Clip::new(
                    "a",
                    "noise",
    ...
    fn a_clip_changes_lane_by_naming_another_one() {
        let mut piece = piece();
        piece.clip_mut("b").expect("it is here").lane = "noise".into();
        assert_eq!(piece.clips_on("noise").count(), 2);
        assert_eq!(piece.clips_on("tone").count(), 0);
        // And nothing else moved: the placement is the clip's own.
        assert_eq!(piece.clip("b").expect("still here").place.offset, 96_000.0);
    }

    /// **The extent is where the last clip ends**, not where it starts: a
    /// ruler and a scroll size themselves against the piece, and a clip dragged
    /// past everything lengthens it by its whole length.
    #[test]
    fn the_extent_is_the_furthest_end_and_not_the_last_onset() {
        let piece = piece();
        assert_eq!(piece.extent(), 144_000.0);
        assert_eq!(Arrangement::default().extent(), 0.0);
    }
    ...
    fn the_stack_lays_the_lanes_at_their_own_heights() {
        ...
        assert_eq!(piece.content_height(4.0), 164.0);
        assert_eq!(Arrangement::default().content_height(4.0), 0.0);
    }
    ...
    fn each_list_reports_as_a_set_take_would_read_it() {
        let mut piece = piece();
        piece.lanes[0].gain = 0.5;
        piece.lanes[0].mute = true;

        let Value::Array(lanes) = piece.lanes_json() else {
            panic!("an array");
        };
        assert_eq!(lanes.len(), 12, "six per lane");
        assert_eq!(lanes[0], Value::from("noise"));
        assert_eq!(lanes[3], Value::from(1), "muted");
        assert_eq!(lanes[5], Value::from(0.5));

        let Value::Array(clips) = piece.clips_json() else {
            panic!("an array");
        };
        assert_eq!(clips.len(), 12, "six per clip");
        assert_eq!(clips[0], Value::from("a"));
        assert_eq!(clips[1], Value::from("noise"), "the lane it is on");
        assert_eq!(clips[3], Value::from(48_000.0));
    }
