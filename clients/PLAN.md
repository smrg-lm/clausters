...
- ✅ **C5 — seq** *(Completado 2026-06-17 — ver NOTAS.md)*: `event`, `pattern`, stream-patterns; un mismo `Pbind`+`TempoClock`+`Server` corre en **RT** (interfaz UDP, servidor vivo) o **NRT** (interfaz score → `render()`) **solo cambiando la interfaz de comunicación de la `Server`**.
  ...
  - ⏳ **Sueltas menores del follow-up (pendiente)**: **golden de paridad de score** más completo cliente↔servidor; devolver la **ergonomía de defaults sin globales** (contexto explícito / fábricas) y, al portar **SynthDef**, construcción **instance-based** (defs concurrentes).
- **C6 — Anclaje del sample-clock por UDP**: lector del reloj de muestras real del servidor **sobre UDP** (query `/clock` + modelo DLL/extrapolación contra el reloj local) para usar `SampleClockTimebase` en vivo sin shm/embed (hoy solo se ancla fácil por `Clausters.clock`/`ShmClient.clock`). Completa el timebase sample-clock para el transporte UDP. Tests de exactitud/jitter (ver `examples/sample_clock.py` del servidor como referencia).
- **C7 — Interfaces MIDI**: completar `_midiinterface`. `MidiNrtInterface`/`MidiScore` con **escritura a archivo MIDI estándar (.mid)** para partituras; `MidiRtInterface` con un **backend real** (p. ej. python-rtmidi, dependencia opcional) para salida en vivo. Mapear `Event` → mensajes MIDI (note on/off por `sustain`, canal, velocity). Misma costura RT/NRT que OSC, vía la interfaz que posee la `Server` (o un destino MIDI análogo). MIDI no lleva timetags: el timing lo da el reloj al emitir.
- **C8 — Interfaz TCP**: `OscTCPInterface` en el cliente (OSC length-prefixed) — hoy es stub. **Requiere soporte TCP en el servidor** (`clausters`, trabajo en el track `M`): el servidor todavía no habla TCP. Dependencia cruzada cliente↔servidor.
- **C9 — multi-lenguaje + cierre**:
  - ✅ **Arquitectura cross-lenguaje documentada**: capítulo nuevo en el mdBook (`docs/clients.md`, en SUMMARY bajo "Library & Embedding") — el contrato C-ABI único (`clausters-core`/`clausters-ffi` + embed/shm), el cliente Python (capas base/seq/defs, estado C0–C5), el camino al cliente **JS** (mismo C-ABI vía N-API/wasm, generators/async en vez de `yield`; sin implementarlo aún) y el plan de **distribución** (wheels Python, npm/wasm JS, Faust en `third_party`). **Confirmación del reuso**: Python (lenguaje no-Rust) ya maneja todo el sistema por el C-ABI + OSC, prueba de que la frontera no es Python-específica.
  - ⏳ **Cierre pendiente**: ejemplo comentado en `examples/` del cliente; cliente **JS** real (N-API/wasm); empaquetado wheels/npm + build reproducible de Faust en `third_party`.
