...
- ✅ **C10 — Mantenimiento de documentación y ejemplos** *(barrido al día hasta C9, 2026-06-17 — ver NOTAS.md; sigue activo a medida que avancen nuevos milestones)*: mantener al día el mdBook (`docs/`), `clients/python/README.md`, la `GUIA.md` del cliente (pasos + conteos) y los ejemplos. Hecho en este barrido: refrescados los estados «C0–C5» → «C0–C9» (README, `docs/clients.md`), catalogados `synthdef.py`/`tcp_client.py` en `docs/examples.md`, agregada la fila C9 al checklist de la GUIA, y **documentada la clase `SynthDef` tal como quedó** (post-orden topológico + dedup, controles por nombre, **solo operadores `+-*/`** componen UGens, outputs deben ser UGens). `mdbook build` limpio.
