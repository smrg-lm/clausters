...
- **C9 — multi-lenguaje + cierre**:
  ...
  - ⏳ **Pendiente**: cliente **JS** real (N-API/wasm; mismo C-ABI, generators/async en vez de `yield`); empaquetado wheels/npm + build reproducible de Faust en `third_party` (este último ya es backlog del usuario). Ambos son esfuerzos grandes e independientes del cierre docs/ejemplos.
