...
- ✅ **C5 — seq** *(Completado 2026-06-17 — ver NOTAS.md)*: `event`, `pattern`, stream-patterns; un mismo `Pbind`+`TempoClock`+`Server` corre en **RT** (interfaz UDP, servidor vivo) o **NRT** (interfaz score → `render()`) **solo cambiando la interfaz de comunicación de la `Server`**.
  ...
  - ⏳ **Timebase alternativo (follow-up)**: el monotónico (default) ya está, con el hook `timebase` (callable) que deja la alternativa **enchufable**. Falta implementar el **sample-clock del servidor** como timebase seleccionable y los **tests robustos con ambas opciones** (el usuario lo enmarcó como "cuando se implemente"). Ídem un **golden de paridad de score** más completo cliente↔servidor.
