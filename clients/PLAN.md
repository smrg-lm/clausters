...
- **C7 — Interfaces MIDI** *(replanificado y subsumido en `PLAN.md` M17 del workspace; diferido)*: completar `_midiinterface`. `MidiNrtInterface`/`MidiScore` con **escritura a archivo MIDI estándar (.mid)** para partituras; `MidiRtInterface` con un **backend real** para salida en vivo. Mapear `Event` → mensajes MIDI (note on/off por `sustain`, canal, velocity). Misma costura RT/NRT que OSC, vía la interfaz que posee la `Server` (o un destino MIDI análogo). MIDI no lleva timetags: el timing lo da el reloj al emitir. **Decisión revisada**: el MIDI no va en una librería de Python (python-rtmidi) ni solo en el cliente, sino en un **crate nativo reusable cliente+servidor** (`crates/clausters-midi`, C ABI versionada), con la capa de mensajes en **MIDI 2.0/UMP vía `midi2`** (alta resolución: velocity 16-bit, controllers 32-bit, `no_std`/no-alocante) y la escritura `.mid` (SMF, MIDI 1.0) vía `midly`. Ver **M17** del `PLAN.md` raíz para el alcance completo (protocolo MIDI del servidor + salida del cliente).
