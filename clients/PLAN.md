...
- ✅ **C5 — seq** *(Completado 2026-06-17 — ver NOTAS.md)*: `event`, `pattern`, stream-patterns; un mismo `Pbind`+`TempoClock`+`Server` corre en **RT** (interfaz UDP, servidor vivo) o **NRT** (interfaz score → `render()`) **solo cambiando la interfaz de comunicación de la `Server`**.
  ...
  - ⏳ **Sueltas menores del follow-up**: **golden de paridad de score** más completo cliente↔servidor; devolver la **ergonomía de defaults sin globales** (contexto explícito / fábricas) y, al portar **SynthDef**, construcción **instance-based** (defs concurrentes).
...
- **C9 — multi-lenguaje + cierre**: confirmar reuso del C-ABI desde JS (nota N-API/wasm, sin implementarlo aún); docs (mdBook: capítulos nuevos en `docs/`), `GUIA.md` (pasos manuales + conteos), ejemplo comentado en `examples/`, `NOTAS.md`.
