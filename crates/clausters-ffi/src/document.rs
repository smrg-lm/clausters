...
///
/// # Safety
/// `widths` must be null or readable for `widths_len` bytes, and `out` null or
/// writable for `out_cap` bytes.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn clausters_mixer_defs(
    widths: *const u8,
    widths_len: usize,
    master: usize,
    out: *mut u8,
    ...
    // SAFETY: forwarded from this function's own contract.
    let Some(widths) = (unsafe { text(widths, widths_len) }) else {
        return 0;
    };
