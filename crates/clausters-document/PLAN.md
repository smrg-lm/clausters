...
- ⬜ **O30 - The host edits a multitrack alone.** The requirement that settled where the projections live, taken as a milestone: `clausters-gui` opens a session, edits it and sounds it with no client in the process. It links the same crate rather than growing a fourth implementation, which is the whole of why the order above is worth its cost. **Acceptance:** a multitrack edited and played from the standalone host, and the same session opened afterwards from either client with no conversion.

- ⬜ **O31 - A join over fragments mints a source.** *(Asked for twice by use: 2026-09-11, a take cut into pieces with edges pulled, shuffled and joined; 2026-09-12, the same thing at its minimum -- one cut, two halves, swapped. `clients/gui/PLAN.md`, "Join over fragments", is the report; this is the answer.)*
...
- ⬜ **O32 - The applications crate: the multitrack application, once, in Rust.** *(Decided 2026-09-13 by the user, after the by-ear check of the standalone host following `C56` (`clients/python/PLAN.md`): no ruler, no transport buttons, a space bar that logs a roll and sounds nothing, and a join that stitches an empty buffer -- "Yo pasaría todo el ejemplo de Python a Rust. La regla se compone, no es parte del widget multitrack", "el standalone tiene que hacer la composición que hace el cliente Python en el ejemplo edit_multipista".)* This is how `O30` is delivered, and `O30`'s acceptance is this milestone's.
  ...
  **Steps, in order** *(decided 2026-09-13 with the user: port the Python client step by step, since it is the one known to work best)*. Each leaves both clients and the standalone host green.
  ...
  2. ⬜ **The conversation and the history.** An application state that reads a report (`conversation`, `intake`), applies it to the piece with its inverse, walks undo and redo, and answers with the acknowledgement and the corrections — what `Editor`, `Application`, `Editing` and `Echo` do today for a piece. The host's `answer_own` arms for a piece go.
  3. ⬜ **Playing, from the window.** The transport row's verbs (rewind, play/pause, stop), the space bar, a locate on the ruler, and the clock's reading, over `PiecePlayback`, answered as steps **addressed** to the server that holds the samples or the one that sounds. The host's `roll_piece`/`cue_piece` go.
  4. ⬜ **A source an edit mints.** A join stitched where the samples are and attached where the piece sounds, through the same addressed steps; for a client both are one server.
  5. ⬜ **Entering a box.** The double click that opens a box's contents in the editor for its structure, on the piece's own history.
  6. ⬜ **The clients hold a handle.** `MultitrackEditor` in both clients forwards reports and carries the answers, holding no composition and no event wiring; the acceptance below is checked by a recorded exchange.
...
- ⬜ **A join's crossfade is a rule this crate invented, and it has to be stated and settled** *(raised by the user 2026-09-12, on the first working join: "le agregaste un cross fade al join, es algo que esta pero no se ve y deberia, puede ser una opcion de join pero tiene que estar especificada y ser configurable")*. `O31` fades every seam **where the material is cut** — 10 ms, `picture::SEAM`, and nothing where two parts read on from each other. Both halves of that are decisions, and the user is right that neither was asked for:

  - **It is not seen.** The fade is in the source, and a box is drawn as a window onto a source, so the picture shows a join and not the shape of its seams. Every other fade this piece has — a region's `fade_in`/`fade_out` — is drawn where it is, which is exactly what makes it editable. A seam that only exists in the recipe is a decision the hand cannot find, let alone change.
  - **It is not stated.** 10 ms is a number this crate chose, in a place with no vocabulary for choosing one: a join carries the fade, so the fade travels, so nothing can disagree about it between clients — which is right, and which is also why a *default* nobody named is the wrong kind of right. It belongs to the join: an option with a length and a shape, whatever `Fade` already is, defaulted rather than fixed.
  - **And the rule about *when* is the same question.** "Cut, so fade; continuous, so do not" is a good default and it is still a rule that should be sayable — a comping pass wants every seam soft, a cut put back together wants none.

  **The rule as it stands today, written down so it can be found and changed** *(`crates/clausters-document/src/multitrack/picture.rs`, `SEAM` and `read_join`)*. It is a default nobody asked for, and it is stated here rather than left to be rediscovered in the code:

  1. A seam between two parts is faded **only where the material is cut** — where the next part does not read on from where the one before it stops, or reads a different source. Two parts that are consecutive pieces of one take get **no** fade, on the grounds that a fade there is audible damage to material that was continuous.
  2. The fade is **10 ms**, linear, on both sides of the seam: `fade_out` on the part that ends and `fade_in` on the one that begins.
  3. It is the same number in every client, because the join carries it — which is the one part of this that is not up for discussion: whatever the fade is, it travels with the source, so nothing can disagree about it.

  **What has to become sayable** is each of those: a join **with or without** a crossfade at all, a crossfade of any **duration**, and of any **shape** — `Fade` is already a length and a curve, and a seam is the same object a region's own fade is. Per join at least, and per seam is what a comping pass actually wants: the same run of fragments may need a long soft cross at one seam and a butt cut at the next.

  Not a milestone yet because how a fade *inside* a box is drawn is a picture question the multitrack has not had to answer, and it is worth answering once for this and for the crossfade of an overlap that the gap and mix cases will need.
