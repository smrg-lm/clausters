...
- ⬜ **The instance is the third projection, and it is the one still written twice** *(proposed by the user 2026-09-11, after a pass in which the same four defects had to be fixed in two languages and two of them turned out never to have crossed at all)*. The crate already holds the discipline for the two projections that are pure: an edit is an `Intent` and [`intent::apply`] is the only thing that applies one; a picture is `multitrack::picture` and a node plan is `multitrack::nodes::plan`. Both are functions of a document, so both are written once. The **instance** -- what is actually sounding on a server, and the diff that makes it match the plan -- is not a function of the document: it is state, it owns ids, buses and buffers, and it is therefore the one projection that was left to each client. So it is written twice (`clients/python/clausters/gui/editing/playback.py` and `clients/web/src/gui/editing/playback.ts`), and drift there is invisible to every compiler and every test: `handPorts` and `staysPut` lived in Python alone for a day, and a page went on taking automations back with a plain set and leaving a dragged box sounding through the track it came from.

  **The shape, and it is the reconciler's.** A declarative tree is diffed against a committed one and the difference is emitted as primitive operations, with the *diff* shared and only the primitives per platform -- React's reconciler and its `HostConfig`, SwiftUI's, Elm's. Here the plan is the tree, `Instance` is the committed one, and the primitives are four OSC messages. So: `multitrack::instance::Instance`, holding node ids, bus runs, buffer numbers and the last ports sent, keyed by the document's own ids; `Instance::reconcile(&plan) -> Vec<Op>` as a pure function with no I/O; and a client that owns the socket, the allocators and nothing else. The reason to state it as `Op` rather than as bytes is that it is then testable offline, usable by a standalone host with no client in the loop, and identical under NRT.
