...
- ⬜ **A domain answers one question today and owes four** *(proposed by the user 2026-09-11; widened by them the same day from the clip to every structure the three endpoints share)*. Every editable structure in this system is one model with **three endpoints** -- the document that owns it, the host that draws it, the server that sounds it -- and an edit may start at any of them. That is an MVC whose legs are messages, and the crate already decided the discipline for it: a projection has **one** implementation, in Rust, and an endpoint only carries. What is not finished is the set of projections that discipline covers.

  **The four, and the score.** (1) *Edit* -- a gesture or a script call becomes an `Intent` in the structure's own vocabulary, applied by `intent::apply` alone: **done**, and it is the reason three clients cannot mean three things by one edit. (2) *Undo* -- the inverse and the coalesce key: **done**, through `domain::coalesce_key`. (3) *View* -- the structure as the props a host draws: **half**, since `multitrack::picture` is shared but the flattening into the wire's flat arrays is written per client, and nothing at all is shared for points, samples or events. (4) *Instance* -- what is sounding, and the diff that makes it match: **not started as shared code**; `nodes::plan` is a pure function of the document and is shared, and everything after it is state, so it was left to each client.

  So `domain.rs` -- the table that exists because otherwise "every binding would spell every domain's rule again" -- answers exactly one of the four questions it is the natural seat for. The argument that put it there is the same argument for the other three, and the evidence is counted rather than asserted: `gui/editing` is ~4,500 lines of Python against ~5,350 of TypeScript, twin for twin (`domain` 109/118, `points` 193/241, `samples` 250/323, `events` 370/484, `playback` 497/663, `multitrack` 983/1208), and no compiler or test reads either against the other.

  **Authority per endpoint, which is the rule the messages need and nothing states yet.** The document is authoritative for the model; the server for what is *sounding* (node ids, bus runs, buffer contents); the host for **view state alone** -- zoom, scroll, selection, which curve is showing. Stating it is not bookkeeping: the selection bug of 2026-09-11 was a model correction destroying view state, and it is only nameable as a defect once each endpoint's own is named. The corollary is the invariant a projection has to hold: **a correction carries model and never view.**

  **Where it lives, settled by what the host has to be able to do alone** *(the user, 2026-09-11)*. The question looked open -- the *view* projection produces the props of the GUI protocol, which is `clients/gui`'s and not the document's, and the crate does not draw. It is settled by the requirement rather than by taste: **a standalone host must be able to edit a multitrack with no client in the process at all**, which `clients/gui/src/host/document.rs` and `--session` already half do. A host that edits needs all four projections, so none of them may sit in a layer above it. And the objection dissolves on inspection: **props are JSON, not types** -- a client builds a dict of primitives and flat arrays and the host parses a `Map<String, Value>` -- so producing them needs no dependency on the host crate and no knowledge of a renderer. So the projections go in a crate beside the document (`clausters-editing`), which the host links directly and every client binds through the FFI and wasm doors it already has.

  **What is actually duplicated, measured rather than assumed** *(2026-09-11, over `clients/gui/editing`, 4,574 lines of Python against 5,350 of TypeScript)*. The view projection is **12%** of it. The largest single duplicated block is not a projection at all but the **conversation**: `Editor` is 865 lines of Python and 1,023 of TypeScript implementing one protocol -- read a report, check it against the version, turn it into intents, apply, then acknowledge or answer with the picture -- and it is an algorithm, not glue. Then view (561), edit ingestion (525) and instance (455). What is irreducibly per-language is the rest: sockets and async, the id/bus/buffer allocators, and writing a payload back onto the client's own objects. So the order by payoff is the conversation first, then the three projections, and the shells that remain are transport plus allocation.

  **The instance projection is the one with state, and its shape is the reconciler's**, so it is the one worth designing first. A declarative tree is diffed against a committed one and the difference is emitted as primitive operations, with the *diff* shared and only the primitives per platform -- A declarative tree is diffed against a committed one and the difference is emitted as primitive operations, with the *diff* shared and only the primitives per platform -- React's reconciler and its `HostConfig`, SwiftUI's, Elm's. It is also what every monolithic DAW does with one process and no names for it: a change marks the graph dirty and one pass rebuilds it. Here the plan is the tree, `Instance` is the committed one, and the primitives are four OSC messages. So: `multitrack::instance::Instance`, holding node ids, bus runs, buffer numbers and the last ports sent, keyed by the document's own ids; `Instance::reconcile(&plan) -> Vec<Op>` as a pure function with no I/O; and a client that owns the socket, the allocators and nothing else. The reason to state it as `Op` rather than as bytes is that it is then testable offline, usable by a standalone host with no client in the loop, and identical under NRT.

  **What it needs from the server, and it is one verb.** A clip changes track by being made again today, which is not a client's fault: a slot instance's wiring is baked into its `out` controls when it is instantiated, so moving the node would leave it writing the track it came from. The monolithic answer -- the one the user named, where changing track moves a pointer -- is `/graph_moveSlot`: re-parent the node and re-bake the wiring of that one slot. It also retires the rebuild's fallout (the map a curve loses with the node, the readers made again, the envelope that came back at the def's default).

  **What it does not become.** Not an object per structure that both draws and updates. The host draws and must not learn the document; the crate holds the document and must not learn a renderer. What the monolith gets from putting both on one object is that *one* piece of code owns the transition, and here that is the projection, not the structure. The drift this removes is worth naming exactly: `handPorts` and `staysPut` lived in Python alone for a day, and a page went on taking automations back with a plain set and leaving a dragged box sounding through the track it came from.
